...
To make precise the equivalence of Theorem~\ref{thm:traced as cob-alg} we will specify here how to construct a $(\Cat{Cob}/\Ob\cat{C})$-algebra from a traced category $\cat{C}$.  First, given a set $Z$ over $\Ob\cat{C}$ we will abuse notation slightly and denote by the same symbol $Z$ the tensor of all objects in $\cat{C}$ labeling elements of $Z$.  Then for an oriented 0-manifold $X=X^-\sqcup X^+$ over  we set $P(X)=\Hom_{\cat{C}}(X^-,X^+)$.  Given a cobordism $\Phi:X\to Y$ we may find $A,B,C,D,E\in\Ob\cat{C}$ such that $X^-\cong C\otimes A$, $X^+=C\otimes B$, $Y^-\cong A\otimes D$, $Y^+\cong B\otimes D$ and $E$ is the tensor of all labels on floating loops in $\Phi$, thus $\Phi$ is essentially equivalent to the cobordism below.  With this notation, for $f\in P(X)$ we define $P(\Phi)(f)=\Tr_{A,B}^C\big[f\big]\otimes D\otimes\dim(E)$.  This can be visualized via the following string diagram (where we have drawn a left trace).
$$
\begin{tikzpicture}[wiring diagram, baseline=(current bounding box.center)]
  \node[bb port sep=1.5, bb={2}{2}, bb name=$f$] (domain) {};
  \node[bb={2}{2}, fit={(domain) ($(domain.north)+(0,1)$) ($(domain.south)-(0,1)$)}] (codomain) {};
  ...
  \draw[ar] let \p1=(domain.north east) in
          (\x1+.7*\bbx,\y1) to[in=0] (\x1+.7*\bbx,\y1+\bby) -- (\x1+.6*\bbx,\y1+\bby) to[out=180] (\x1+.6*\bbx,\y1) -- (\x1+.7*\bbx,\y1);
...
A technical proof of \ref{thm:Theorem A} will be given in Section~\ref{sec:first equivalence} (see Corollary~\ref{cor:traced as cob-alg}), but see Section~\ref{sec:first equivalence} for an intuitive picture-proof.
...
It turns out that if our compact category $\cat{C}$ is the compact closure of a traced category $\cat{D}$, then \ref{thm:Theorem B} lifts to a result about $\cat{D}$. We record this result as \ref{cor:Corollary B}, from which \ref{thm:Theorem A} follows. These are proved in Sections~\ref{**},~\ref{**}, and~\ref{**}, respectively. 
...
%\subsection{Scalars and elements}\label{sec:scalars and elements}\todo{Do we use this section: either scalars or elements?}
%
%Let $(\cat{M},\otimes,I)$ be a symmetric monoidal category where for any $X,Y\in\Ob(\cat{M})$ we write $\gamma_{X,Y}\colon X\otimes Y\To{\sim} Y\otimes X$ for the distinguished symmetry isomorphisms. Recall that we have $\gamma_{Y,X}=\gamma_{X,Y}^{-1}$.  We define the {\em monoid of scalars in $\cat{M}$} to be the set $S_{\cat{M}}\coloneqq\Hom(I,I)$ with multiplication given by composition; in fact $S_{\cat{M}}$ is a commutative monoid.  
%
...
%We write $|X|\coloneqq\Hom(I,X)$ for the set of \emph{elements} of $X$. The functor $|\cdot|=\Hom(I,\cdot)\colon\cat{M}\to\Set$ is an $\cat{M}$-algebra, i.e., a lax monoidal functor, with unit map
...
\end{tikzpicture}
$$
This can be recognized as a cobordism between oriented 0-manifolds, as we will discuss in Section~\ref{sec:Cob morphisms as wds}. Each of the six axioms is vacuous from this perspective, in the sense that both sides of the equation correspond to the same cobordism. For example, here are the axioms of \emph{dinaturality} and \emph{superposition}:
\begin{itemize}
...
\end{description}
%ERASED
For an object $X$ in a traced category $\cat{M}$ we write $\dim(X)\coloneqq\Tr^X_{I,I}\big[\id_X\big]\in S_{\cat{M}}$ for the {\em dimension} of $X$.  Note also that for any endomorphism $f\colon X\to X$ there is a scalar $s_f=\Tr^X_{I,I}[f]\in S_{\cat{M}}$.
%ERASED
...
\todo{Reference here that $\Fun{Int}(\Cat{Bij})=\Cat{Cob})$.}
\todo{Move stuff to appendix?}
...
with unit object $\tilde I\coloneqq(I,I)$ and duality $(\inp{X},\outp{X})^\vee\coloneqq(\outp{X},\inp{X})$.  The following is immediate from the definitions above and in Section~\ref{sec:scalars and elements}.
...
\section{Free traced and compact categories, and geometry}

\begin{proposition}

Let $\cat{C}$ be a category. The free traced category on $\cat{C}$ is... The free compact category on $\cat{C}$ is $\Fun{Int}(...)$.\todo{Fill this in}
...
\begin{proof}
See Kelly, Laplaza ``Coherence for compact closed categories'' and Freyd, Yetter ``Braided compact closed categories with applications to low dimensional topology''
\end{proof}
...
\begin{proposition}
  There is a contravariant pseudo-functor $\XCat\to\Cat{Cat}$ sending any $\cat{C}\in\XCat$ to the category $\cat{C}/\XCat_{\bo}$.
\end{proposition}
\begin{proof}
...
Given a category $\cat{C}$, there is a monoidal category $\Prof(\cat{C},\cat{C})$ of profunctors from $\cat{C}$ to itself and morphisms of profunctors, with the tensor product given above and $\Hom_{\cat{C}}$ as the monoidal unit. Moreover, this construction is functorial, being the object part of a functor
\[
  \EndoProf\colon \op{\Cat{Cat}} \to \MonCatLax.
\]
To see the action of this functor on morphisms, consider a functor $F\colon\cat{C}\to\cat{D}$. We want to define a lax functor 
\[
  F^*\colon\Prof(\cat{D},\cat{D})\to\Prof(\cat{C},\cat{C}).
\]
Given a profunctor $M\in\Prof(\cat{D},\cat{D})$, define $F^*(M)\coloneqq M(F,F)$. If $\phi\colon M\Rightarrow N$ is a profunctor morphism in $\Prof(\cat{D},\cat{D})$, define $F^*(\phi)\colon M(F,F)\to N(F,F)$ by sending $m\in M(Fc_1,Fc_2)$ to $\phi(m)\in N(Fc_1,Fc_2)$.
...
\]
	It follows that if $\cat{C}$ is compact then there is an equivalence of categories
\[
\Mon(\MProf(\cat{C},\cat{C}))\iso \cat{C}/\CompCat_{\bo}.
\]
...
\todo{This builds on Lemma~\ref{lemma:CCprofunctors}}
\end{proof}
...
In this section, we prove \ref{thm:Theorem B}, from which \ref{cor:Corollary B} and \ref{thm:Theorem A} follow (see in particular Sections~\ref{sec:proof of Bs}~and~\ref{sec:proof of A}). We offer the following diagram as a guide through this section.
\[
\begin{tikzcd}[column sep=3cm,row sep=1.5cm]
\Lax(\cat{C},\Set)\ar[r,dashed,"\tn{\ref{thm:Theorem B}}"]\ar[d,equal]&\cat{C}/\CompCat_{\bo}\\
\MProf(1,\cat{C})\ar[r,"\tn{Proposition~\ref{Prop:mon prof equivalence}}"']&\Mon(\MProf(\cat{C},\cat{C}))\ar[u,"\tn{Lemma~\ref{Lem:mon profs equal boo functors}}"']
\end{tikzcd}
\]
If $\cat{C}$ is a compact category, the top equivalence in the diagram above is \ref{thm:Theorem B}, which we will defined as the composite of the other three equivalences. Our first goal, then, is to prove Proposition~\ref{Prop:mon prof equivalence}, which is the purpose of Section~\ref{sec:ccc's}. 
...
We need to show that this equivalence is natural in the following sense. Suppose that $F\colon\cat{D'}\to\cat{D}$ is a strong traced functor, and let $\cat{C'}\coloneqq\Int(D')$. We want to show that the diagram 
$$
\begin{tikzcd}
...
and this equivalence is natural in $\cat{O}$.
...
\Tr^{G\otimes M}_{K,L}\big[f\big]\otimes J\otimes H\otimes \dim(\overline{N\otimes S\otimes T})
\end{align}
