...
\begin{equation}\begin{tikzpicture}[wiring diagram]
...
\cite{HK}, and let $\TrCat$ denote the underlying 1-category of $\TTrCat$ \todo{In footnote, mention
tilde's, $\MMonCatStrong$?}%
\footnote{
...
\todo{DR: Can we change ``make sense in'' to something like ``should describe'', since
$\ncat{Tr'Cat}$ is not yet defined?} $\ncat{Tr'Cat}$. For example, one may want to consider
cartesian traced categories, in which wires can split or terminate abruptly. Or one might want to
allow splitting but not allow abrupt termination.
...
requiring surjectivity but not injectivity in $\varphi$. \todo{DR: Should \cite{RupelSpivak} be
mentioned here?}
...
\begin{lemma}
   ...
   \begin{enumerate}[label={\upshape\roman*}.]
      \item The unit $\cat{C}\to\Int(\cat{C})$ is fully faithful. It follows that
      ...
      \item If $\cat{D}$ is a monoidal category and $F\colon\cat{D}\to\cat{C}$ is a fully faithful
         ...
         functor.
      \item If $\cat{C}$ is compact then the unit \todo{DR: Should this be written more precisely as
         ``the unit of the 2-adjunction above''?}
...
\todo{DR: This section title feels misleading, geometry makes me think algebraic- or differential-,
but we don't really say anything about geometry.}
...
\begin{definition}
   ...
   If moreover $F_0\colon\dcat{X}_0\to\dcat{Y}_0$ is fully faithful, we say that $F$ is a
...
\begin{definition}
   ...
   \]
   and call the \emph{equipment induced by $F_0$}. If $\dcat{Y}$ is an equipment, $F_0^*(\dcat{Y})$
   will be too, because fibrations are stable under pullback. By
   Remark~\ref{rem:strict_vs_pseudo_pullback}, the obvious equipment functor
   $F_0^*(\dcat{Y})\to\dcat{Y}$ is a local equivalence.
\end{definition}
...
\begin{definition}
   ...
   $\Ptd(\dcat{D})\twoheadrightarrow\dcat{D}_0$, whose objects are proarrows $A\colon d\tickar d$ in
   $\dcat{D}$ equipped with a unit from \eqref{eqn:unit_and_mult}, but not a multiplication, and
   whose morphisms are 2-cells (as in a monoid homomorphism) which preserve the units.
...
\begin{theorem}
   ...
   If an equipment $\dcat{D}$ is regular \todo{DR: Regularity of equipments was never defined.} (in
   particular, if $\dcat{D}$ is exact) then the vertical 2-category $\VVer(\dcat{D})$ admits a
...
\begin{proof}
   ...
   inverse equivalence $\dcat{D}^\bo_0\to\Mon(\dcat{D})$ sends a $\bo$ map to its kernel.
...
\section{Internal presheaves and copresheaves}
...
\begin{definition}
   ...
   \footnote{In fact, this definition makes sense for any object of $\dcat{D}_0$, but for our
   purposes we require the terminal object. }
   We define the categories $\CPsh(\dcat{D})$ and $\Psh(\dcat{D})$, each bifibered over
   $\dcat{D}_0$, by the pullbacks
   \begin{equation*}
     \begin{tikzcd}
...
$\CPsh(\dCompProf)\equiv\dCompProf^{\bo}$. Similar results hold for traced categories; see
...
}\todo{DR: This does not really seem proper since there can be monoidal profunctors between
symmetric categories which are not symmetric.  Perhaps it should be stated that all monoidal
profunctors are assumed to be symmetric.}
...
Example~\ref{ex:monoid_in_Prof} such that
\begin{equation}
...
\begin{proof}
   ...
   structure, which on objects is just that of $C$, and on morphisms is induced \todo{DR: Do you
   really mean to say that the monoidal structure is induced from a fact?} from the fact that $M$ is
   a monoidal profunctor. It is also easy to verify the second part of
...
\begin{proposition}
   ...
   Let $\dcat{D}$ be $\dProf$, $\dMonProf$,$\dTrProf$, or $\dCompProf$, each of which is exact
   \todo{This assertion doesn’t really belong as part of the Proposition, perhaps it should be moved
   into the proof when it is used.} . A vertical morphism $F$ in $\dcat{D}$ is in $\bo$ (resp.
   $\ff$) in the sense of Theorem~\ref{thm:orthogonal} if and only $F$ is bijective-on-objects (resp.
   fully-faithful) in the usual sense.
\end{proposition}
\begin{proof}
   ...
   Now we consider $\dTrProf$. By the construction of collapses in \eqref{eqn:collapses_in_traced},
   \todo{D: Patrick, clarify} a vertical morphism $F\colon T\to T'$ is in $\bo$ if and only if both
   $\Int(F)$ and the underlying monoidal functor $\UTM(F)$ are in $\bo$. By
...
\begin{remark}
   ...
   and monoids in profunctors act like reflexive and transitive relations.
...
\end{proposition}
\begin{proof}
   The first claim follows directly from Theorem~\ref{thm:Mod_vs_bo} and Lemma~\ref{lemma:FFLE_Mod}.

   Assume $U(\bo)\subseteq\mathrm{iso}(\cat{S})$.
...
\begin{definition}
   ...
   that are free-on-objects.
\end{definition}

So for example, $\MMonFrObCat$ is the vertical 2-category of $\dMonProf_{\FM}^{\bo}$, as defined in
Definition~\ref{def:DFbo}.
...
\end{corollary}
\begin{proof}
   This follows from Proposition~\ref{prop:objectfree_Mod_bo}, letting $\cat{S}=\Set$ and
   Definition~\ref{def:free_on_objects}.
\end{proof}

We are now in a position to connect the introductory notation of \eqref{eq:single_fiber_tr} and
\eqref{eq:single_fiber_comp} to that of the present section, using the following pseudo-pullback
squares; see Lemma~\ref{lem:Mon_pullback} and Theorem~\ref{thm:Mod_vs_bo}.
\begin{equation} \begin{tikzcd}
      \label{eqn:2pullbacks_1fiber}
   \TrCat_{\LabSet}\equiv\CompCat_{\LabSet}\ar[r]\ar[d]\ar[rd,phantom,"\lrcorner" very near start]
    &\dTrProf^\bo\ar[d,"\cod"]\ar[r]\ar[rd,phantom,"\lrcorner" very near start]
...
\begin{proof}
   ...
   \end{equation}
   The result follows from Corollary~\ref{cor:TrCat_ObjectFree}, which provides isomorphisms of
   2-categories:
   \[
      \MMon(\dFrTrProf)\iso\TTrFrObCat
      \quad\text{and}\quad
      \MMon(\dFrCompProf)\iso\CCompFrObCat
   \]
\end{proof}
...
\[
   ...
   \CCompFrObCat \to \CCompCatStrong,
\]
free-on-objects and strict functors vs. arbitrary objects and strong functors.  This result will not
be new to experts, but we found it difficult to find in the literature.
...
$\mu$ to be the composite $I_A\To{\epsilon^{-1}}gf(I_A)=g(I_B)$, \todo{D: Patrick, our definition of
surjective equivalence is the opposite of the standard one. Change it and make corresponding change
here?} and define $\mu_{x,y}$ to be the composite
\[
...
\begin{proof}
   ...
   fact, this will hold as long as $A$ is free-on-objects. We say that $A$ is \emph{flexible} in
   this case, i.e.,~if every strong morphism out of $A$ is isomorphic to a strict morphism.\todo{DR:
   We never actually use this term anywhere else, remove this sentence?}
   ...
   \todo{DR: The abuse of notation replacing $U$ by $U\iota$ was stated in the proof of
   Theorem~\ref{thm:strong_classifier}, is this convention still in effect?  It might be good to
   reiterate the convention here.}, there is a unique morphism $\tilde{f}\colon\Disc(s)\to UA$ such
   that $f=\epsilon_A\circ Ff$. Factoring $\epsilon_A=q_A\circ r_A$ as in
...
\begin{thebibliography}{XXX}
 ...
 \bibitem{Abadi}
 J.~Abadi, On the Existence of an Orthogonal Factorization System on 1-Cob and 2-Cob, \url{http://arxiv.org/abs/1506.03119} (2015).
