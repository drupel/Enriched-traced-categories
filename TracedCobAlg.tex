...
\begin{definition}\label{def:copresheaves}
   Let $\dcat{D}$ be an equipment with a terminal object 1. We define the category $\CPsh(\dcat{D})$,
   bifibered over $\dcat{D}_0$, and similarly $\Psh(\dcat{D})$ by the pullbacks
   \begin{equation}
     \begin{tikzcd}
...
\begin{lemma}\label{lem:Psh_pullback}
   ...
   \begin{equation*}
      \begin{tikzcd}
         \CPsh(\dcat{X}) \ar[r] \ar[d,two heads] \ar[dr,phantom,"\lrcorner" very near start]
            & \CPsh(\dcat{Y}) \ar[d,two heads] \\
         \dcat{X}_0 \ar[r]
            & \dcat{Y}_0
      \end{tikzcd}
   \end{equation*}
   is a pullback of categories, and similarly for $\Psh$.
\end{lemma}
...
of $F$ (the ``discrete'' objects). We will write $\varphi_T\colon\dcat{D}_T\to\dcat{D}$ for the
...
hence the induced $\dMod(\varphi)\colon\dMod(\dcat{D}_T)\to\dMod(\dcat{D})$ is as well. If
