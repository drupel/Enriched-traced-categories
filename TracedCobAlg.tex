...
\begin{proof}
   ...
   $\dCompProf$. Let $(i_M,\vec{\imath}_M)\colon (\Int(T),M)\to\Col{M}_C$ be the collapse embedding
   of $M$ in $\dCompProf$.
   
   We will show that the 1-cell component $i'_M\colon T\to\Col{M}_T$ of the collapse embedding of
   ...
   \begin{equation}\label{eq:Emb_Tr}
      \begin{tikzcd}[column sep=large]
         ...
         \Col{M}_T \ar[r,hook] & \UCT\Col{M}_C.
      \end{tikzcd}
   \end{equation}
   Note that by Lemma~\ref{lem:Tr_bo_Int}, $\Col{M}_C\iso\Int(\Col{M}_T)$.
...
%   \end{equation*}
   ...
   $\Emb_{\mathrm{Tr}}(M,T')\iso\TrCat(\Col{M}_T,T')$, showing that $\Col{M}_T$ is the collapse of
