...
\begin{lemma}
   ...
   Let $F\colon\dcat{D}\to\dcat{E}$ be an equipment functor. If $F$ is a local equivalence then so is $\dMod(F)\colon\dMod(\dcat{D})\to\dMod(\dcat{E})$.  If, moreover, $F$ is fully faithful then so is $\dMod(F)$.
\end{lemma}
