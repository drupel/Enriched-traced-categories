...
\begin{remark}
   ...
   equational form presented. We will use the bimodule notation discussed in Sections~\ref{sec:profunctors}~and~\ref{sec:monoidal_profunctors}, but we highly recommend that the interested reader convert the equations
   ...
   tensor of morphisms. We do not draw these pictures here, because they look too much like pictures in $\Cob$; indeed, that is what we are trying to prove.
\end{remark}
...
\end{proposition}
\begin{proof}
   Let $N\colon C\tickar C$ be a profunctor and $i\colon\Hom_C\to N$ be the basepoint. We can define a multiplication on $N$ by the following formula: given any $n_1\in N(c,d)$ and $n_2\in N(d,e)$,
   \[
   ...
   \begin{align*}
      ...
      &= \phi(n_2)\bullet\phi(n_1)\qedhere
   \end{align*}
\end{proof}
