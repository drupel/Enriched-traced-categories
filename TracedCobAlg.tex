...
In many equipments, there are ways of constructing objects satisfying vertical universal properties from data in the horizontal bicategory. In this paper, we will make extensive use of such a
...
\begin{example}
  ...
  While the exactness of $\dProf$ follows for formal reasons, as we saw in
...
bijective-on-objects functors out of $\cat{C}$ by Theorem~\ref{thm:Mod_vs_bo}.
These
...
This is the purpose of Section~\ref{sec:monoids_on_free}. Finally in Section~\ref{sec:characterization_of_traced} we give our "traceless characterization of $\TTrCat$". 
...
Our goal is to prove Theorem~\ref{thm:mon_prof_equivalence}, an equivalence between two fibrations: copresheaves and monoids. To do so we introduce a third fibration---that of pointed
