...
denote its underlying 1-category.\todo{Comment on underlying 1-category}%
\footnote{
...
\todo[inline]{Move factorization system stuff to Section~\ref{sec:factorization_systems}?}
...
Let $\scat{D}$, $\pcat{D}$, and $\ccat{C}$ be 2-categories, and let $U\colon\pcat{D}\to\ccat{C}$ and $\iota\colon\scat{D}\to\pcat{D}$ be 2-functors. We say that the collection $(\scat{D},\pcat{D},\ccat{C},U,\iota)$ is \emph{strictifiable} \todo{change?}if it satisfies the following properties:
\begin{enumerate}
...
\begin{remark}
   Other than those involving bijective-on-objects or fully-faithful morphisms, all of the properties enumerated in Definition~\ref{def:strictifiable} (namely, Properties~\ref{item:ioofflff}, \ref{item:arrowMPO}, \ref{item:left2adj}, \ref{item:pres_MPO}, \ref{item:reflids}, and \ref{item:createSEs} \todo{correct?}) hold
...
\end{itemize}
We will see that in each case the collection is strictifiable. 
...
$$
The requisite equations can be checked by direct computation, or see \cite{Kelly}.
...
\begin{theorem}\label{thm:strong_classifier}
   Suppose that $(\scat{D},\pcat{D},\ccat{C},U,\iota)$ is strictifiable. Then the functor $\iota$ has a left 2-adjoint $Q\colon\pcat{D}\to\scat{D}$. The
...
\begin{theorem}\label{thm:object-free_strong_equivalence}
   ...
   $F\colon\ccat{C}\leftrightarrows\VVer(\dcat{D}):\!U$ all satisfying the assumptions of
   Section~\ref{sec:strict_vs_strong}.
