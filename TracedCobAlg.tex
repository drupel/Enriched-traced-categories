...
To connect the geometry (string diagrams) with the algebra (free compact categories), we recall the following theorem of Freyd and Yetter.

\begin{theorem}\label{thm:free_compact_is_Cob}
   The free compact category $\FC(\cat{O})$ on a set $\cat{O}$ is equivalent to $\LCob{\cat{O}}$.
\end{theorem}
\begin{proof}
   In \cite[Theorem 3.6]{FreydYetter}, Freyd and Yetter prove something more general: for any set $\cat{O}$, the free braided pivotal category on $\cat{O}$ is the category of $\cat{O}$-colored oriented tangles (up to regular isotopy). The result follows by symmetrizing both sides, i.e., by adding that $\sigma_{B,A}\circ\sigma_{A,B}=\id_A$ for all $A,B$.
...
\end{align*}
given by (\ref{dia:TMvsFrMonCat}), Theorem~\ref{thm:free_compact_is_Cob}, Definitions~\ref{def:copresheaves},~\ref{def:FrMonProf_FrCompProf},~and~\ref{def:FrMonCompProf}, and Lemma~\ref{lem:FrMonCompProf_Psh_Mon}.
\end{proof}
