...
\begin{definition}
   ...
   \begin{compactitem}
      ...
      \item A category $\dcat{D}_1$, equipped with two functors $L,R\colon\dcat{D}_1\to\dcat{D}_0$, called the \emph{left frame} and \emph{right frame} functors. Given an object $A\in\Ob\dcat{D}_1$ with $c=L(A)$ and $c'=R(A)$, we say that $A$ is a \emph{proarrow from $c$ to $c'$} and write $A\colon c\tickar c'$. A morphism $\phi\colon A\to B$ in $\dcat{D}_1$ is called a 2-cell, and is drawn as follows, where $f=L(\phi)$ and $f'=R(\phi)$:
         \begin{equation*}
            \begin{tikzcd}
            ...
            \end{tikzcd}
         \end{equation*}
         There is also a \emph{unit} functor $U\colon\dcat{D}_0\to\dcat{D}_1$, which is a retract of both $L$ and $R$, i.e., $L\circ U=\id_{\dcat{D}_0}=R\circ U.$
       \item A functor $\odot\colon\dcat{D}_1\times_{\dcat{D}_0}\dcat{D}_1\to\dcat{D}_1$, called \emph{horizontal composition}, that is strictly unital in the sense that both of the following composites are identity on $\dcat{D}_1$:
$$
\begin{tikzcd}
	\dcat{D}_1\ar[r,shift left,"U\times_{\dcat{D}_0}\dcat{D}_1"]\ar[r,shift right,"\dcat{D}_1\times_{\dcat{D}_0}U"']&[1.5em]\dcat{D}_1\times_{\dcat{D}_0}\dcat{D}_1\ar[r,"\odot"]&[-1em]\dcat{D}_1
\end{tikzcd}
$$       
Horizontal composition is required only to be weakly associative, meaning that there is a natural isomorphism between the two evident functors $\dcat{D}_1\times_{\dcat{D}_0}\dcat{D}_1\times_{\dcat{D}_0}\dcat{D}_1\too\dcat{D}_1$.
        \end{compactitem}
An \emph{equipment functor} $\dcat{D}\to\dcat{E}$ consists of functors $\dcat{D}_0\to\dcat{E}_0$ and $\dcat{D}_1\to\dcat{E}_1$ commuting with the frames $L,R$, the unit $U$, and the composition $\otimes$. 
\end{definition}
...
\begin{example}
   ...
   \begin{gather}\label{eq:Prof_monoid}
...
\begin{theorem}\label{thm:orthogonal}
   If an equipment $\dcat{D}$ is exact (and hence regular)\todo{Why is this "and hence regular" thing here?}, then the vertical 2-category
