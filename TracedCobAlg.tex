...
For many equipments $\dcat{D}$, monoids in $\dcat{D}$ are category-like objects where the
vertical and horizonal arrows in $\dMod(\dcat{D})$ behave much like functors and profunctors. The canonical
example is the equipment of spans of sets whose objects, vertical, and horizontal arrows are sets,
functions, and spans respectively. In this case, the equipment of monoids is equivalent to $\dProf$, which
may be easily verified by direct calculation.

One natural question is thus: how should one think about monoids in equipments like $\dProf$ whose objects are already
category-like? It turns out that in these cases where $\dcat{D}$ is an equipment of category-like objects,
the equipment $\dMod(\dcat{D})$ of monoids in $\dcat{D}$ is very closely related to $\dcat{D}$ itself. In
other words, monoids in an equipment are category-like objects, but taking monoids in an already
category-like equipment doesn't change much.
...
$M$ in an an exact equipment, there is a corresponding object called the \emph{collapse} of $M$
...
\begin{example}
  ...
  Collapses of monoids always exist in $\dProf$, and this is the prototypical example of callapse.
  ...
  The collapse $\Col{M}$ is thus the category with the same objects as $\cat{C}$, with morphisms
...
Section~\ref{chap:equipments_monoidal_profunctors} begins with an outline of our proof
of \ref{thm:TheoremA} as a sequence of equivalences. To finish the present section,
we establish the following two results that provide the key equivalence in that proof.
These are not about traced or compact categories specifically, but in fact
hold for any exact equipment and form a precise bridge between profunctors and
bijective-on-objects functors. They can be seen as the strongest statement of the idea (mentioned in
...
The key ingredient in the proof of \ref{thm:TheoremB} which we prove in this section (and which implies the corresponding equivalence for traced
categories) is specific to traced and compact categories is the following equivalence of fibrations
...
\begin{lemma}
  ...
  Let $\cat{C}$ be a compact category. Any pointed endo-profunctor $i\colon\Hom_\cat{C}\to M$ in
  $\Ptd(\dCompProf)$ induces a natural bijection $M(a,b)\iso M(I,a^*\otimes b)$ for any objects
  $a,b\in \cat{C}$.
\end{lemma}
\begin{proof}
