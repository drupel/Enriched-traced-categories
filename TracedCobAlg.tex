...
Write $\TrCat^{\bo}$ for the full subcategory of the arrow category $\TrCat^{\to}$ spanned by the bijective-on-objects functors.  The existence of this factorization system implies that the domain functor
...
of compact categories.  The Int construction, applied to a traced category $\cat{T}$, returns the
smallest compact category $\Int(\cat{T})$ of which it is a monoidal subcategory. We will recall these
...
Our eventual understanding of this result will pass through the 2-categorical notion of \emph{equipments}; see Section~\ref{chap:background_equipments} and the introduction
to Section~\ref{chap:equipments_monoidal_profunctors}.
...
\begin{lemma}
   ...
   is an object.  For $N=\Mon(F)(M)$ and $d=F_0(c)$ we have a pullback square in $\Set$, natural in
   $M$ and $c$:
      \[ \begin{tikzcd}
...
\begin{proof}
   ...
   fibrations since $\vec{f}$ being cartesian over $f$ implies $(f,\tilde{f})$ is as well. The
...
In order to properly understand the full equipment $\dcat{D}^{\bo}$ we need to introduce the equipment of modules and bimodules in $\dcat{D}$.
\begin{definition}
...
As we have seen in Section~\ref{sec:exactness_and_boff} the exactness of an equipment gives rise to many equivalences of fibrations, thus to establish our main theorems we aim first to show that the equipments from Section~\ref{sec:monoidal_profunctors} are all exact.
...
$\FrMonCat\coloneqq\dFrMonProf_0$ (and similarly for traced and compact) for the vertical
