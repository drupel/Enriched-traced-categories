...
In fact, these generalize \todo{according to Wikipedia this is not accurate}Penrose diagrams in $\Cat{Vect}$~\cite{}, and the word \emph{traced} originates in this vector space terminology.
...
\begin{tikzcd}
	...
	\Prop\ar[r]\ar[d,"\Ob"']\arrow[dr, phantom, "\lrcorner", very near start]&\SymMonCat_1\ar[d,"\Ob"]\\
	\Set\ar[r,"\Fun{List}"']&\Set
\end{tikzcd}
\]
where $\SymMonCat_1$ (resp. $\TrCat_1$) is the underlying 1-category of $\SymMonCat$ (resp.\ of $\TrCat$), and where $\Fun{List}\colon\Set\to\Set$ is the free monoid monad. This definition of $\Prop$ agrees with that in~\cite{HackneyRobertson}.\todo{Check this.}
...
\todo{Introduce equipments and refactor.}
