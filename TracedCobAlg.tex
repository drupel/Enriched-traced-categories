...
of this left adjoint, and which are satisfied by all of our motivating examples.\todo{David: flesh this out.}
...
\begin{definition}\label{def:preserve_jdetect_strictness}
 ...
 Suppose that $i\colon\ccat{C}\to\ccat{D}$ is a 2-functor that is identity-on-objects, faithful, and locally fully-faithful. We say that the triple $(\ccat{C},\ccat{D},i)$ \emph{has mapping path objects} if $\ccat{D}$ has mapping path objects as in Definition~\ref{def:mapping_path_objects} such that
 \begin{itemize}
 \item for any $f\colon a\to b$ in $\cat{D}$, the structure morphisms $a\From{\pi_a}P(f)\To{\pi_b}b$ are in $\ccat{C}$, and
 \item for a diagram in $\cat{D}$ of the following form
 $$
      \begin{tikzcd}[column sep=small]
         & |[alias=domA]| x \ar[dl,"g"'] \ar[dr,"h"] & \\
         a \ar[rr,"f"' codA] && b
      ...
      \end{tikzcd}
  $$
  we have that $g,h$ are in $\ccat{C}$ if and only if the induced map $\hat{\alpha}\colon x\to P(f)$ is in $\ccat{C}$.
\end{itemize}
