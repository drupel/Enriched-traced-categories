...
The proof of this result can be found in Section~\ref{sec:deducing}. It is useful to see how the free-on- objects traced (resp. compact) categories relate to general traced categories. In an appendix, we prove the following result, which is well-known but difficult to find in the literature:
...
constructs in detail, and the fact that these extend to 2-adjunctions will be proved in
...
The Kleisli categories $\Set_{\TM}$, $\Set_{\TT}$, and $\Set_{\TC}$ are equivalent to the full subcategories of $\MonCat$, $\TrCat$, and $\CompCat$ (respectively) spanned by objects which are free on a set. Note that there is an isomorphism of categories $\Set_{\TM}\iso\Set_{\TT}$, because $\TM=\TT$.
...
\begin{example}
   In the case of $\dFrMonProf\coloneqq\dMonProf_{\TM}$, Proposition~\ref{prop:objectfree_Mod_bo}
   provides an equivalence $\dMod(\dFrMonProf)\equiv\dMonProf^{\bo}_{\FM}$. We can think of
   $\dMonProf^{\bo}_{\FM}$ as the equipment of free-on-objects monoidal categories. Indeed, this
   just says that a free-on-objects monoidal category can be identified with a set $\LabSet$, a
   monoidal category $\cat{C}$, and a bijective-on-objects strict monoidal functor
   $\FM(\LabSet)\to\cat{C}$.
   \todo{Prove equivalence with $\MMonFrObCat$, etc.?}
...
\begin{corollary}\label{cor:TrCat_ObjectFree}
   ...
   respectively.\todo{Should remove right column.}
\end{corollary}
\begin{proof}
   This follows directly from Lemma~\ref{lem:free-on-obs_inclusion} and
   Theorem~\ref{thm:free_on_objects_strong_equivalence}, taking $\ccat{C}$ to be $\CCat$ for the first
...
\begin{remark}
   ...
   an alternative \emph{definition} of the 2-category $\TTrCat$. This definition does not involve any explicit mention of the trace structure defined in \cite{JoyalStreetVerity} (recalled in Section~\ref{sec:intuition_for_traced}).
\end{remark}
...
\appendix
\chapter{Appendix: strong vs. strict functors}
\setcounter{secnumdepth}{1}

<<<<<<< HEAD
In the appendix, our goal is to prove Corollary~\ref{cor:object_frees}. We do not believe it is a new result, but had difficulty finding a reference. Here we give a self-contained proof.
=======
A \emph{monoidal profunctor morphism} $\phi\colon M\to N$ is simply a monoidal transformation.
Spelling this out in bimodule notation, $\phi$ is an ordinary morphism of profunctors such that
$\phi(m_1\boxtimes m_2)=\phi(m_1)\boxtimes\phi(m_2)$ and $\phi(I_M)=I_N$. One can define 2-cells
...
\section{Fully faithful / bijective-on-objects factorization, and surjective equivalences}
...
$\dFrMonProf$ is just the full sub-equipment of $\dMonProf$ spanned by the monoidal categories which
are free on a set, and similarly for the other two. We will write
$\FrMonCat\coloneqq\Ver(\dFrMonProf)$, etc.
<<<<<<< HEAD
>>>>>>> origin/new_version
=======
>>>>>>> origin/new_version
...
Our goal in this section is to prove Corollary~\ref{cor:object_frees}, which says that the 2-category of monoidal categories and strong functors is equivalent to that of monoidal categories that are free-on-objects and strict functors; similarly for traced and compact categories.
=======
>>>>>>> origin/new_version

We continue the assumptions of the Theorem~\ref{thm:strong_classifier}, that $\pcat{D}$ is
...
and only if $\Ob f$ is an isomorphism. We will write $\ccatFrOb{D}$ for the full sub-2-category of
...
\begin{proof}
   ...
   $\VVer(\dcat{D})(A,B)\iso\pcat{D}(\iota A,\iota B)$ for any $A$ and $B$ which are
   free-on-objects. In fact, this will hold as long as $A$ is free-on-objects. We say that $A$ is
   \emph{flexible} in this case: every strong morphism out of $A$ is isomorphic to a strict
   morphism.

   If $A\in\dcat{D}$ is free-on-objects, then there exists an object $s\in\cat{S}$ and a $\bo$
...
<<<<<<< HEAD
   ...
   Finally, essential surjectivity, consider an object $D\in\pcat{D}$. We know that in the
   factorization
=======
\end{proof}

\begin{corollary}\label{cor:TrCat_ObjectFree}
   There are fully-faithful local equivalences of equipments
   \begin{align*}
      \dMod(\dFrMonProf)  &\to \dMonProf \\
      \dMod(\dFrTrProf)   &\to \dTrProf  \\
      \dMod(\dFrCompProf) &\to \dCompProf
   \end{align*}
   by means of which we can identify $\MMonFrObCat=\MMon(\dFrMonProf)$, etc.
\end{corollary}
\begin{proof}
   The fully-faithful local equivalences follow immediately from
   Lemma~\ref{lem:free-on-obs_inclusion}, letting $\cat{S}=\Set$. The identification with
   $\MMonFrObCat$, etc., follows directly from Proposition~\ref{prop:objectfree_Mod_bo} and
...
Say that a monoidal (resp. traced or compact) category is \emph{free-on-objects} if there is a set $\cat{O}$ and a bijective-on-objects functor $\FM(\cat{O})\onto\cat{C}$, (resp. $\FT(\cat{O})\onto\cat{C}$ or $\FC(\cat{O})\onto\cat{C}$), where $\FM$, $\FT$, and $\FC$ are the free functors defined in \eqref{eqn:adjunctions}. Denote by
$$
