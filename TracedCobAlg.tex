...
of category $\ncat{Tr'Cat}$, one may proceed by imagining the string diagrams that should describe compositions in $\ncat{Tr'Cat}$. For example, one may want to consider
...
\end{tikzpicture} \end{equation*}
There is sometimes a tension between identity morphisms and the trace structure, e.g., if the morphisms are dynamical systems. Imagining the wiring diagrams for traced categories without identities is straightforward (the relevant operad is $\cat{L}$ above), whereas determining the relevant axiomatization for them is not. 

\todo[inline]{DS: Include something like the following?\\An approach to this issue, was suggested in \cite{ABP}, in which traced categories without identities are embedded in larger categories as ``traced ideals"; however, it may sometimes be preferable to work with the category on its own terms.}
...
requiring surjectivity but not injectivity in $\varphi$. These were found to be useful in \cite{RupelSpivak}. 
...
\begin{definition}
   ...
   We refer to an equipment functor $F\colon\dcat{X}\to\dcat{Y}$ as a \emph{local equivalence} if
   ...
   \begin{equation} \begin{tikzcd}
      ...
      \dcat{X}_0\times\dcat{X}_0 \ar[r,"F_0\times F_0"']
...
\begin{definition}
   ...
   functor $\dMod(F)\colon\dMod(\dcat{C})\to\dMod(\dcat{D})$.
\end{definition}

If $M$ and $N$ are monoids, we denote by $\Bimod{M}{N}$ the 1-category of $(M,N)$-bimodules and
bimodule morphisms.
...
\end{tikzcd} \end{equation}
in which the triangle invlolving the $F$'s and the triangle involving the $U$'s both commute.
...
\begin{thebibliography}{XXX}
 ...
 \bibitem{ABP}
 S.~Abramsky, R.~Blute, P.~Panangaden, Nuclear and Trace Ideals in Tensored ????Categories, arXiv preprint math/9805102 (1998).
