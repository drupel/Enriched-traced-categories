...
products; we refer to this latter condition using the term \emph{objectwise-free}. Much of the work
in this paper goes towards relaxing these two conditions.

To handle objectwise-freeness, we prove that the 2-category of traced categories and
...
We denote fibrations of 1-categories using two-headed arrows $\onto$.
...
\begin{remark}
   ...
   It is a standard fact for 1-categories that a strict pullback of a Grothendieck fibration (or opfibration) along an arbitrary functor is a pseudo-pullback. Thus if the square \eqref{eqn:local_equiv} is a strict pullback, it will be a pseudo-pullback, and hence a local equivalence. Any local equivalence can be replaced by an equivalent strict pullback, which we define in Definition~\ref{def:induced_locally_equivalent_equipment}.
...
\begin{example}
  ...
  equipment $\dMod(\dcat{D})$ of monoids and bimodules is exact. Since there is an equivalence $\dProf\cong\dMod(\dSpan)$~\cite{Shulman}, where $\dSpan$ is the equipment of spans in $\Set$, the equipment $\dProf$ is exact.
\end{example}
...
\begin{proof}
  ...
  the isomorphism $\conj{i}_M\otimes_M\comp{i}_M\iso\unit(\Col{M})$ by restricting both sides:
  \begin{align*}
