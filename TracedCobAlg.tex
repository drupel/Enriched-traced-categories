...
The category $\Lax(\cat{C},\Set)$ of lax set-valued functors out of a compact category $\cat{C}$ have interesting behaviors. For example, when $\cat{C}$ is free on a set $\cat{O}$ of objects, lax functors can be identified with object-free traced categories, as in \eqref{eq:single_fiber}, or with object-free compact categories:
\begin{equation*}
