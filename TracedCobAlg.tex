...
\begin{definition}
   ...
   $f$ and $g$}, written $M=N(g,f)$.  By an \emph{equipment functor}, we simply mean a strong double functor between equipments.
\end{definition}
...
\begin{definition}
   ...
   the following square strictly commutes and is a pseudo-pullback of categories:
   \begin{equation} \begin{tikzcd}
   ...
   \end{tikzcd} \end{equation}
   where the left-hand and right-hand arrows are the frame fibrations $(L,R)$.
...
\end{corollary}
\begin{proof}
   Since $\Int\colon\dTrProf\to\dCompProf$ is a local equivalence and preserves the terminal object, Lemma~\ref{lem:Mon_pullback} and Lemma~\ref{lem:Psh_pullback} give a factorization of the desired equivalence of fibrations through that of Theorem~\ref{thm:mon_prof_equivalence}.
\end{proof}
...
Consider the fibration $\End(\dCompProf)\onto\CompCat$, defined by the strict pullback
\[ \begin{tikzcd}[column sep=0em]
...
\begin{proof}
   ...
   from~Theorem~\ref{thm:mon_prof_equivalence}, and Lemmas~\ref{lem:Mon_pullback}
