...
\subsection{Wiring diagrams and $\Cob$}\label{sec:wds and cob}
...
$$\LCob{\cat{O}}\alg\simeq\TrCat_{\cat{O}}$$
from Section~\ref{sec:statement_of_main_thm}. We will show that the same data are required, and the same conditions are
...
We conclude this section by briefly specifying how to construct a
$(\Cob/\cat{O})$-algebra $P$ from a traced category $(\cat{C},\otimes,I,\Tr)$ whose objects are freely generated by $\cat{O}$. We will abuse notation slightly as follows: Given a relative set
...
set $P(X)=\Hom_{\cat{C}}(\inp{X},\outp{X})$. Given a cobordism $\Phi\colon X\to Y$, there exist
...
\begin{equation}\label{eq:cob algebra formula}
   P(\Phi)(f)=\Tr_{A,B}^C(f)\otimes D\otimes\Tr^E_{I,I}(\id_E).
\end{equation}
...
\begin{align*}
   \FrM\colon\Set &\leftrightarrows\MonCat:\!\UM \\
   \FrT\colon\Set &\leftrightarrows\TrCat:\!\UT \\
   \FrC\colon\Set &\leftrightarrows\CompCat:\!\UC
\end{align*}

\todo[inline,author=Patrick]{Talk about $\FrMonCat$, etc.\ and the induced $\FrMonCat\to\FrCompCat$}
