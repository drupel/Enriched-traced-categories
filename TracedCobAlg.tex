...
fully-faithful functor $\Set_{\TT}\to\Set^{\TT}$ from the Kleisli to the Eilenberg-Moore categories,
...
In Section~\ref{sec:monoidal_profunctors} we introduce various equipments of monoidal profunctors
($\dMonProf, \dTrProf$, and $\dCompProf$), our main objects of study, which we prove are all exact
...
briefly records an intersting byproduct of the theory developed for the proofs of the main theorems:
...
profunctors}, which are profunctors between monoidal categories with extra structure. Just as when
studying monoidal/traced/compact categories and their corresponding functors and natural
transformations, a bit of 2-category theory is often useful, when studying monoidal profunctors
between such structured categories and their relation to functors and natural transformations, it is
...
All of the types of structured categories of interest in this paper can be organized into
...
For many equipments $\dcat{D}$, monoids in $\dcat{D}$ are category-like objects, and moreover the
vertical and horizonal arrows in $\dMod(\dcat{D})$ are like functors and profunctors. The canonical
example is the equipment of spans of sets, whose objects, vertical and horizontal arrows are sets,
functions, and spans. In this case, the equipment of monoids is equivalent to $\dProf$, as is not
difficult to verify directly.

One natural question then is: what are monoids in equipments like $\dProf$, whose objects are already
category-like? It turns out that in cases where $\dcat{D}$ is an equipment of category-like objects,
the equipment $\dMod(\dcat{D})$ of monoids in $\dcat{D}$ is very closely related to $\dcat{D}$. In
other words, monoids in an equipment are category-like objects, but taking monoids in an already
category-like equipment doesn't add much new.
...
some basic results. In Section~\ref{sec:exactness_proofs} we will prove that the equipments of
interest are exact. This will allow us to use the collapse construction as a step in the connection
between lax monoidal set valued functors---which will be proarrows in exact equipments---with
(traced or compact) monoidal categories---which will be objects of those exact equipments. We will
also need to make use of fully-faithful/bijective-on-objects factorization systems, which we show in
this section exist in any exact equipment. (In \cite{Schultz2015} it is shown that this
factorization system exists in any \emph{regular} equipment, as the image regular-epi/mono
...
\begin{remark}
  ...
  this preserves horizontal composition, defining a double functor, and hence a local equivalence.
\end{remark}
...
In the introduction to Section~\ref{chap:equipments_monoidal_profunctors} we will outline our proof
of \ref{thm:TheoremA} as a sequence of equivalences. To finish
Section~\ref{chap:background_equipments} we prove a proposition and theorem which provide the key
equivalence in that proof which is not about traced or compact categories specifically, but in fact
holds for any exact equipment, and which forms the bridge between profunctors and
...
equivalences of fibrations over $\CompCat$ and $\TrCat$ respectively:
\begin{equation*}
...
Section~\ref{sec:exactness_proofs}. The parts of the proof which are specific to traced and compact
categories go into establishing the middle equivalences, which we do in
Section~\ref{sec:special_CompProf}.
...
The key ingredient in the proof of \ref{thm:TheoremB} which is specific to traced and compact
categories is an equivalence of fibrations
\[ \begin{tikzcd}[column sep=-.5em]
  \CPsh(\dCompProf) \ar[rr,"\equiv"] \ar[dr,two heads,"\MOb"']
    && \Mon(\dCompProf) \ar[dl,two heads,"\MOb"] \\
  & \CompCat &
\end{tikzcd} \]
which we prove in this section (and which implies the corresponding equivalence for traced
categories). To do so we introduce a third fibration---that of pointed endo-proarrows---and
...
The next lemma shows that the standard natural equivalence
$\Hom_{\cat{C}}(a,b)\equiv\Hom_{\cat{C}}(I,a^*\otimes b)$ for any compact category $\cat{C}$ can be
