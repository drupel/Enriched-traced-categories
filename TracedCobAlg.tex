...
Note that we have the following:\vspace{-.2in}
\begin{center}
   \parbox{1.8in}{
      \begin{align*}
         \MonCat&=\dMonProf_0\\
...
{\centering
   ...
   \parbox{2.8in}{
      \begin{align*}
         \dFrObMonProf&\coloneqq\dMod(\dFrMonProf)\\
         \dFrObTrProf&\coloneqq\dMod(\dFrTrProf)\\
         \dFrObCompProf&\coloneqq\dMod(\dFrCompProf)
      \end{align*}
   }
}\\
We refer to the equipments in the second column as \emph{object-free}.
\end{definition}
...
\begin{corollary}\label{cor:TrCat_ObjectFree}
   ...
   \begin{align*}
      \VVer(\dFrObMonProf)&\equiv\MMonCat \\
      \VVer(\dFrObTrProf)&\equiv\TTrCat \\
      \VVer(\dFrObCompProf)&\equiv\CCompCat.
   \end{align*}
\end{corollary}
   ...
   \begin{equation*}
      \begin{tikzcd}[column sep=-1.5em]
         \CPsh(\dFrCompProf) \ar[rr,"\equiv"] \ar[dr,two heads]
            && \dFrObCompProf_0 \ar[dl,two heads] \\
            & \FrCompCat &
      \end{tikzcd}
   \end{equation*}
\end{lemma}
