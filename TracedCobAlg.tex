...
\end{equation}
Some intuition for this statement will be given in Section~\ref{subsec:cobalg_and_trCat}, and it will be generalized in \ref{thm:TheoremA} and proven as such in Section~\ref{sec:deducing}.
...
A strong functor $F\colon \cat{C}\to \cat{C'}$ between object-free traced categories sends each object in $\cat{C}$ to the tensor product of finitely many objects in $\cat{C'}$; that is, it induces a function $\Ob F\colon\Ob\cat{C}\to\TM(\Ob \cat{C'})$, where $\TM\colon\Set\to\Set$ is the free monoid monad. Let $\Set_{\TM}$ denote the Kleisli category of this monad, so $\Ob F$ is identified with a morphism in $\Set_{\TM}$. There is a functor $(\LCob{\bullet})\colon\Set_{\TM}\to\CompCat$ (see~\eqref{dia:TMvsFrMonCat}~and~\eqref{eqn:FMC}), \todo{Patrick: check this paragraph; is $(\LCob{\bullet})$ clear enough notation?}and we can compose it with $\Lax(-,\Set)$ to obtain a functor which we denote
\begin{equation}\label{eqn:cob/bullet}
(\LCob{\bullet})\alg\colon\op{\Set_{\TM}}\too\Cat.
\end{equation}
By applying the Grothendieck construction to (\ref{eqn:cob/bullet}), we collect the equivalences (\ref{eq:single_fiber}) for varying $\cat{O}$ into a single functor, and we have the following main result, whose proof can be found in Section~\ref{sec:deducing}.
...
It is most convenient to prove~\ref{thm:TheoremA} by proving a more general result, which
...
To connect the geometry (string diagrams) with the algebra (free compact categories), we recall the following Theorem  \cite[Theorem 3.6]{FreydYetter} of Freyd and Yetter.

\begin{theorem}\label{thm:free_compact_is_Cob}
For any set $\cat{O}$, the free braided pivotal category on $\cat{O}$ is the category of $\cat{O}$-colored oriented tangles (up to regular isotopy).
\end{theorem}

\begin{corollary}\label{cor:free_compact_is_Cob}
The free compact category $\FC(\cat{O})$ on a set $\cat{O}$ is equivalent to $\LCob{\cat{O}}$.
\end{corollary}
\begin{proof}
  This result follows by symmetrizing both sides of Theorem~\ref{thm:free_compact_is_Cob}, i.e., by adding that $\sigma_{B,A}\circ\sigma_{A,B}=\id_A$ for all $A,B$.
   \todo{Check this.}
\end{proof}
...
\end{itemize}
As shown, we often indcate morphisms in $\cat{L}$ using a two-headed arrow, and morphisms in $\cat{R}$ using a hooked arrow.
\end{definition}
\todo{Move later remark here.}
...
An equipment is a double category $\dcat{D}$ in which the functor $\dcat{D}_1\tto\dcat{D}_0\times\dcat{D}_0$ is a fibration.
\end{definition}
...
\begin{definition}\todo{rephrase in terms of embeddings}
...
Recall the notion of orthogonal factorization systems for categories and strict 2-categories from Definition~\ref{def:orthogonal}.
...
\end{theorem}
\todo{need proofs}
\begin{corollary}\label{cor:Mon_vs_bo}
...
\section{Other stuff}
\todo{Change name of ``Other Stuff" section, once things settle.}

\subsection{Internal presheaves}\label{subsec:internal_presheaves}

Presheaves on a category $\cat{C}$ can be identified with profunctors $1\tickar\cat{C}$ in $\dProf$, and similarly for copresheaves, which are more relevant to us. Motivated by this, we will think of proarrows $C\tickar 1$ in any equipment $\dcat{D}$ with a
terminal object 1 as ``internal copresheaves'' on the object $C$. For each object, there is a category
of copresheaves $\HHor(\dcat{D})(C,1)$. We can give a direct construction of the bifibration over
$\dcat{D}_0$ whose fiber over an object $C$ is the category of copresheaves on $C$:
...
\section{Definitions of equipments}
\todo[inline]{Need definitions}

\begin{definition}\label{def:MonProf}
The equipment $\dMonProf$ is defined as follows.
\end{definition}

\begin{definition}\label{def:TrProf}
The equipment $\dTrProf$ is defined as follows.
\end{definition}

\begin{definition}\label{def:CompProf}
The equipment $\dCompProf$ is defined as follows.
\end{definition}
...
\begin{lemma}\label{lem:Int_preserves_bo}
   If a traced functor $F\colon\cat{T}\to\cat{T}'$ is bijective-on-objects, then so is $\Int(F)$.
\end{lemma}
...
\begin{lemma}\label{lem:Tr_bo_Int}
   ...
   \begin{equation*}
      \begin{tikzcd}[column sep=large]
         \UTM T \ar[r,hook,"\UTM\eta_T"] \ar[d,two heads]
   ...
   \begin{compactitem}
      \item there is a unique trace structure on $M$---defining a traced category $T'$ with $\UTM
         ...
         \begin{equation*}
            \begin{tikzcd}[column sep=large]
               T \ar[r,hook,"\eta_T"] \ar[d,two heads,"G"']
...
\begin{proof}
   ...
   $\dCompProf$. Define $\Col{M}_{\mathsf{C}}$ and $(i_M,\vec{\imath}_M)\colon (\Int(T),M)\to\Col{M}_{\mathsf{C}}$ to be the collapse embedding of $M$ in $\dCompProf$.
   
   We will show that the object $\Col{M}_{\mathsf{T}}$ and the 1-cell component $i'_M\colon T\to\Col{M}_{\mathsf{T}}$ of the collapse embedding of
   ...
   \begin{equation}\label{eq:Emb_Tr}
      \begin{tikzcd}[column sep=large]
         ...
         \Col{M}_{\mathsf{T}} \ar[r,hook] & \UCT\Col{M}_{\mathsf{C}}.
      \end{tikzcd}
   \end{equation}
   Note that by Lemma~\ref{lem:Tr_bo_Int}, there is an isomorphism $\Col{M}_{\mathsf{C}}\iso\Int(\Col{M}_{\mathsf{T}})$.
...
%   \begin{equation*}
%      \begin{tikzcd}
%         \TrCat(\Col{M}_{\mathsf{T}},T')
%               \ar[r,"\eta_{T'}\circ\textrm{--}"]
%               \ar[d,"\textrm{--}\circ i'_M"']
%               \ar[dr,phantom,"\lrcorner" very near start]
%            &
%               \TrCat(\Col{M}_{\mathsf{T}},\UCT\Int(T'))
%               \ar[d,"\textrm{--}\circ i'_M"']
%               \ar[r,"\iso"]
%            &[-.6em] 
%               \CompCat(\Col{M}_{\mathsf{C}},\Int(T'))
%               \ar[dd,"\iso"]
%               \ar[ddl,bend left=9,"\textrm{--}\circ i_M" description]
%            \\
%         \TrCat(T,T')
%               \ar[r,"\eta_{T'}\circ\textrm{--}"]
%               \ar[dr,bend right=9,looseness=1.1,"\Int"' sloped]
%            & \TrCat(T,\UCT\Int(T'))
%               \ar[d,"\iso"'] 
%             \\
%         & \CompCat(\Int(T),\Int(T'))& \Emb_C(M,\Int(T')) \ar[l]
%      \end{tikzcd}
%   \end{equation*}
   \begin{equation*}
      \begin{tikzcd}[row sep={0,between origins},column sep={0,between origins}]
         \TrCat(\Col{M}_T,T')
            ...
            &[2em,between borders]
               \TrCat(\Col{M}_T,\UCT\Int(T'))
               \ar[dd,"\textrm{--}\circ i'_M"']
               \ar[dr,phantom,"\iso" sloped]
            &[6.6em]
            &[6.6em] \\[2.3em]
         && \CompCat(\Col{M}_C,\Int(T'))
         ...
         \TrCat(T,T')
            ...
            &&& \Emb_C(M,\Int(T')) \ar[d] \\[4em]
         &&& \CompCat(\Int(T),\Int(T'))
      \end{tikzcd}
   \end{equation*}
   The top left square is a pullback, by the orthogonality of $i'_M\in\bo$ and $\eta_{T'}\in\ff$.
   Comparing with~\eqref{eq:Emb_Tr}, we get a natural bijection
   $\Emb_{\mathrm{Tr}}(M,T')\iso\TrCat(\Col{M}_{\mathsf{T}},T')$, showing that $\Col{M}_{\mathsf{T}}$ is the collapse of
   ...
   The second part of Definition~\ref{def:exact_equipment} is immediate from exactness of
   $\dCompProf$.
\end{proof}
...
\begin{definition}\label{def:FrMonProf_FrCompProf}
   We define the equipments $\dFrMonProf\coloneqq\dMonProf_{\TM}$ and $\dFrCompProf\coloneqq\dCompProf_{\TC}$. In
   particular, there is a pullback
   \begin{equation*}
      \begin{tikzcd}
...
Advances in Mathematics 77 (1989), no. 2, 156Ð182. 
