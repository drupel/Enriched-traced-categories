...
$M\colon c\tickar c$ to its underlying object $|M|\coloneqq c$. The following result can also be found in \cite{FioreGambinoKock}.\todo{P: Christina suggested we add this reference; thoughts?  It looks like Prop. 3.3 there corresponds to our lemma.}
...
\begin{remark}
  ...
  For any monoid $M\colon c\tickar c$, the companion $\comp{i}_M\colon c\tickar \Col{M}$ (resp.\ the conjoint $\conj{i}_M\colon \Col{M}\tickar c$) of the embedding $i_M\colon c\to\Col{M}$ has the structure of a left (resp.\ right) $M$-module. Indeed, these are given by rewriting the collapse 2-cell $\vec{\imath}_M$ as follows:\todo{I don't understand how to construct these diagrams.  It seems to me that we should consider the horizontal composition of $\vec{\imath}_M$ with the cartesian two-cell defining $\comp{i}_M$ to get a two-cell into $\Col{M}$, which then factors through $\comp{i}_M$ using the cartesian property.}
	\[
	\begin{tikzcd}
      c\ar[r,tick,"M"]\ar[d,equal]&
...
\end{proposition}
\begin{proof}
   Clearly these 2-categories have the same underlying 1-category, so it suffices to show that
...
\todo{Unused references commented, we probably want to incorporate some of these into the main text somewhere.}
