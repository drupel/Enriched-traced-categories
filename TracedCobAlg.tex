...
\begin{example}
   ...
   $$
   \Ob P(F)\coloneqq\{(A,B,i)\suchthat A\in\Ob\cat{A}, B\in\Ob\cat{B}, i\colon F(A)\To{\iso}B\text{ is an isomorphism}\}
   $$
 ...
 If $\cat{M}$ and $\cat{M'}$ are traced categories and $F$ is a traced functor, one obtains a canonical trace structure on the monoidal category $P(F)$ in a manner similar to that shown in Example~\ref{ex:arrow_objects}. If $\cat{M}$ and $\cat{M'}$ are compact categories, then the mapping path monoidal category $P(F)$ is naturally compact: the dual of $(A,B,i)$ is $(A^*,B^*, (i^{-1})^*)$. 
 \end{example}
