...
of category $\ncat{Tr'Cat}$, one may proceed by imagining the string diagrams that make sense in \todo{DR: Can we change ``make sense in'' to something like ``should describe'', since $\ncat{Tr'Cat}$ is not yet defined?}
...
requiring surjectivity but not injectivity in $\varphi$. \todo{DR: Should \cite{RupelSpivak} be mentioned here?}
...
\end{equation}
for $U,X,Y\in\Ob(\cat{M})$ satisfying several, say seven,\todo{DR: Why not simply write ``seven''?} equational axioms. When traced categories
...
\begin{enumerate}[label={\upshape\roman*}.]
   ...
   \item If $\cat{C}$ is compact then the unit \todo{DR: Should this be written more precisely as ``the unit of the 2-adjunction above''?} $\cat{C}\To{\raisebox{-1ex}{$\equiv$}}\Int(\cat{C})$ is an equivalence.
...
\section{Free traced and compact categories, and geometry \todo{DR: This section title feels misleading, geometry makes me think algebraic- or differential-, but we don't really say anything about geometry.}}\label{sec:free_and_geometry}
...
\begin{theorem}\label{thm:orthogonal}
   If an equipment $\dcat{D}$ is regular \todo{DR: Regularity of equipments was never defined.} (in particular, if $\dcat{D}$ is exact) then the vertical
...
\footnote{
   ...
   assumption; see Warning~\ref{warn:symmetric}.\todo{DR: This does not really seem proper since there can be monoidal profunctors between symmetric categories which are not symmetric.  Perhaps it should be stated that all monoidal profunctors are assumed to be symmetric.}
}
...
\begin{proof}
   ...
   which on objects is just that of $C$, and on morphisms is induced \todo{DR: Do you really mean to say that the monoidal structure is induced from a fact?} from the fact that $M$ is a
...
\begin{proposition}\label{prop:boff_well_named}
   Let $\dcat{D}$ be $\dProf$, $\dMonProf$,$\dTrProf$, or $\dCompProf$, each of which is exact \todo{This assertion doesn’t really belong as part of the Proposition, perhaps it should be moved into the proof when it is used.} . A vertical morphism
...
\todo{D: Patrick, our definition of surjective equivalence is the opposite of the standard one.
Change it and make corresponding change here?}
...
\begin{proof}
   ...
   morphism.\todo{DR: We never actually use this term anywhere else, remove this sentence?}
   ...
   adjunction \todo{DR: The abuse of notation replacing $U$ by $U\iota$ was stated in the proof of Theorem~\ref{thm:strong_classifier}, is this convention still in effect?  It might be good to reiterate the convention here.}, there is a unique morphism $\tilde{f}\colon\Disc(s)\to UA$ such that
