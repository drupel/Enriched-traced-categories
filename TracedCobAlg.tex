...
\begin{example}
   The 2-categories $\CCat$, $\MMonCat$, $\TTrCat$, and $\CCompCat$ have arrow objects. Clearly for an object $\cat{C}\in\CCat$, the usual arrow category $\cat{C}^2$ of arrows and commutative squares, has the necessary universal property. If $\cat{M}$ is a monoidal category then $\cat{M}^2$ has a natural monoidal product
   $$
   	\cat{M}^2\times\cat{M}^2\iso(\cat{M}\times\cat{M})^2\To{\otimes^2}\cat{M}^2
   $$
 and because $\cat{M}^2$ is an arrow object in $\CCat$, it is easy to check, using the universal property \eqref{eqn:up_for_arrows}, that $\cat{M}^2$ is an arrow object in $\MMonCat$.
\todo{finish}
\end{example}
