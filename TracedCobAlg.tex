...
Let $\Cat{XKls}$ be the Kleisli category of the monad induced by (\ref{eq:abramsky}). There is an induced fully faithful functor $\Cat{XKls}\to\XCat$ (see \cite[Proposition 4.2.1]{Borceux vol 2}. 
Let $\Fun{XFrObCat}\colon\Cat{XKls}\to\op{\Cat{Cat}}$ be the composite
...
This construction is functorial, in the sense that 
...
$$
\begin{tikzcd}
\Set\ar[d]\ar[r,"\Fun{TrFr}"]&\TrCat\ar[r,"\Fun{Int}"]\ar[dr,"-/\TrCat_{\bo}"']&\CompCat\ar[d,"\alg"]\\
\Cat{TrKls}\ar[ur]&&\Cat{Cat}
\end{tikzcd}
$$
