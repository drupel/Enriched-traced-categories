...
\begin{theorem}\label{thm:ffle_and_verts}
   ...
   $\ccat{C}$, such that $\Ob(U(\bo))\subseteq\mathrm{iso}(\cat{S})$. Let
   $T=\Ob\circ U\circ F\circ\Disc$ be the induced monad on $\cat{S}$.

   Then the composition
   \begin{equation*}
      \begin{tikzcd}
         \MMon(\dcat{D}_T)\ar[r,"\MMon(\varphi_T)"]
            &[1.5em] \MMon(\dcat{D}) \ar[r,"\VVer(\Col{\textrm{--}})"]
            & \VVer(\dcat{D}) \ar[r,"\iota"]
            & \pcat{D}
      \end{tikzcd}
   \end{equation*}
...
\begin{proof}
   ...
   Finally, to see that $\iota\circ G$ is essentially surjective, let $D\in\dcat{D}$ be an object,
   and consider the $(\bo,\ff)$ factorization of the counit
   \begin{equation*}
      \begin{tikzcd}
