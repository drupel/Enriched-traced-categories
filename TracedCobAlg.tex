...
\subsection{The categories $\Prop$ and $\TrProp$ of props and traced props}
...
\end{tikzcd}
$$
where $\SymMonCat_1$ (resp. $\TrCat_1$) is the underlying 1-category of $\SymMonCat$ (resp. of $\TrCat$), and where $\Fun{List}\colon\Set\to\Set$ is the free monoid monad. This definition of $\Prop$ agrees with that in \cite{Robertson}.\todo{Check this.}
...
\todo[author=Patrick,inline]{Introduce framed bicategories, give examples}

\subsection{The monoids and bimodules construction}
\todo[author=Patrick,inline]{Give construction, examples, monad structure}
