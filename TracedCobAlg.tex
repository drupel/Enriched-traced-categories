...
$\TrCat^{\bo}_{\cat{T}/}\coloneqq\dom^{-1}(\cat{T})$ is the category of strong, bijective-on-objects
...
$\TrCat_{\LabSet}\iso\TrCat^{\bo}_{(\FT\LabSet)/}$, and similarly for $\CompCat_{\LabSet}$.
...
and Justin Hilburn for many useful conversations. Thanks also to Tobias Fritz for helpful comments on a draft of this paper.
...
Recall that a \emph{monoidal category} $\cat{C}$ is a category equipped with a functor
$\otimes\colon\cat{C}\times\cat{C}\to\cat{C}$, an object $I\in\cat{C}$, and natural isomorphisms
\begin{gather*}
   a_{X,Y,Z}\colon(X\otimes Y)\otimes Z\to X\otimes(Y\otimes Z) \\
   \lambda_X\colon I\otimes X\to X \\
   \rho_X\colon X\otimes I\to X
\end{gather*}
satisfying some equations; see \cite{MacL--CTWM}. The category $\cat{C}$ is \emph{symmetric monoidal} if
there is in addition a natural isomorphism
\begin{equation*}
   \sigma_{X,Y}\colon X\otimes Y\to Y\otimes X
\end{equation*}
satisfying additional equations.
...
are isomorphisms, then $F$ is \emph{strong}.

Let $\MMonCat$ denote the 2-category of symmetric monoidal categories, strong symmetric monoidal
functors, and monoidal transformations, and let $\MonCat$ denote the underlying 1-category. Let
...
\begin{warning}\label{warn:symmetric}
   For the rest of this article, whenever we discuss monoidal categories, we will mean non-strict
   symmetric monoidal categories. For example, see the definition of $\MMonCat$ above.
\end{warning}
...
If $\cat{C}$ and $\cat{D}$ are traced categories, then a \emph{traced functor} is simply a strong
symmetric monoidal functor which preserves the trace operation.
...
There are free/forgetful adjunctions of 1-categories
\begin{align*}
