...
names regular and exact where chosen. From this perspective, $\dMod(\dcat{D})$ can be seen as the
exact completion of the equipment $\dcat{D}$.
...
this section exist in any exact equipment (as the image regular-epi/mono factorization system exists
in any exact category).
...
\begin{example}
  ...
  While the exactness of $\dProf$ follows for formal reasons, it will be helpful to understand
  collapse in $\dProf$ concretely.
...
\todo[inline]{Patrick add emphasis to \ref{thm:Mod_vs_bo}.}
\begin{theorem}
    \label{thm:Mod_vs_bo}
...
The following is a direct consequence of Theorem~\ref{thm:Mod_vs_bo} and Propositions~\ref{prop:CompProf_exact} and~\ref{prop:TrProf_exact}.
...
In this section we will prove \ref{thm:TheoremB}. \todo[inline]{Patrick, introduce this section?}
...
Our goal in this section is to construct an equivalence of fibrations
\[ \begin{tikzcd}[column sep=-.5em]
  \CPsh(\dCompProf) \ar[rr,"\equiv"] \ar[dr,two heads,"\MOb"']
    && \Mon(\dCompProf) \ar[dl,two heads,"\MOb"] \\
  & \CompCat &
\end{tikzcd} \]
To do so we introduce a third fibration---that of pointed endo-proarrows---and establish its
equivalence with each of $\CPsh(\dCompProf)$ and $\Mon(\dCompProf)$ in
...
Recall that we use the notation $\int$ to denote the Grothendieck construction.
...
monoidal (resp. traced and compact) categories and strict functors between them (see Definition~\ref{def:free_on_objects}), whereas
