...
\begin{corollary}\label{cor:Mon vs bo}
...
\todo[inline]{Formally define $\dMonProf$, $\dTrProf$, and $\dCompProf$ here.}

\section{Exactness}
\todo[inline]{Prove exactness of $\dMonProf$, $\dTrProf$, and $\dCompProf$.}
...
\begin{theorem}\label{thm:Verts}
   ...
   \begin{equation*}
      \MMon(\dcat{D}_T)\equiv\VVer(\dcat{D}).
   \end{equation*}
\end{theorem}
...
\end{named}
\begin{proof}
   $\CPsh(\dCompProf)\equiv\Mon(\dCompProf)$ by~\ref{Prop:mon prof equivalence}, and by~\ref{cor:Mon
   vs bo} we have the equivalence $\Mon(\dCompProf)\equiv\CompCat^{\bo}$.
\end{proof}
