...
\end{equation}
Some intuition for this statement will be given in Section~\ref{subsec:cobalg_and_trCat}, and it will be generalized in \ref{thm:TheoremA} and proven as such in Section~\ref{sec:proof_of_A}.
...
\begin{lemma}\label{lem:Psh_pullback}
   ...
   \begin{equation*}
      \begin{tikzcd}
         \Psh(\dcat{X}) \ar[r] \ar[d,two heads] \ar[dr,phantom,"\lrcorner" very near start]
            & \Psh(\dcat{Y}) \ar[d,two heads] \\
         \dcat{X}_0 \ar[r]
            & \dcat{Y}_0
      \end{tikzcd}
   \end{equation*}
   is a pullback of categories, and similarly for $\CPsh$.
\end{lemma}
...
\begin{definition}
   We define the equipments $\dFrMonProf=\dMonProf_{\TM}$ and $\dFrCompProf=\dCompProf_{\TC}$. In
...
\todo[inline]{With the organization that is forming, these are all easy corollaries of earlier
results. Maybe we don't need this ``main results'' section?}

To prove the main theorems \ref{thm:TheoremA}, \ref{thm:TheoremB}, and \ref{cor:CorollaryB} stated in the introduction, we would first like to restate those theorems
...
\begin{equation*}
	\begin{tikzcd}
		...
		\MMon(\dFrMonCompProf)\ar[r,"\simeq"']&\TTrCat
	\end{tikzcd}
\end{equation*}
The left-hand map is surjective on objects, so it suffices to check that there is an equivalence of categories 
$$\CobKlsAlg\simeq\Mon(\dFrMonCompProf).$$
...
\begin{named}{Theorem A'}
   There is a fully-faithful functor of 1-categories
   \begin{equation*}
      \CPsh(\dFrMonCompProf)\to\TrCat
   \end{equation*}
   such that the composition with the 1-skeleton inclusion $\TrCat\to\TTrCat$ is 2-essentially
   surjective.
\end{named}
\begin{proof}
   Lemma~\ref{lem:FrMonCompProf_Psh_Mon} and Proposition~\ref{prop:TrCat_2equiv}.
\end{proof}
