...
Let $\CCat_\iso$ denote the 2-category (or if one prefers, (2,1)-category) of categories, functors, and natural isomorphisms.
...
\begin{example}\label{ex:mapping_paths}
 ...
 The 1-adjunctions from \eqref{eqn:adjunctions},
 \begin{gather*}
		\FM\colon\CCat\leftrightarrows\MMonCat:\!\UM
\qquad
		\FT\colon\CCat\leftrightarrows\TTrCat:\!\UT
...
\begin{definition}\label{def:preserve_jdetect_strictness}
 ...
 \item the pair $(\pi_a,\pi_b)$ \emph{preserve and jointly detect morphisms in $\ccat{X}$} in the following sense: for any morphism $\ell\colon x\to P(f)$ in $\ccat{Y}$, we have that $\ell$ is in $\ccat{X}$ if and only if the compositions $\pi_a\circ\ell$ and $\pi_b\circ\ell$ are in $\ccat{X}$.
\end{itemize}
...
\begin{enumerate}
 ...
 \item\label{item:left2adj}
   The functor $U\iota\colon\scat{D}\to\ccat{C}$ has a left 2-adjoint $F$ (equivalently,
 ...
 \item\label{item:pres_MPO}
   The functor $U$ preserves mapping path objects and reflects identity 2-cells.
...
\begin{remark}
   Other than those involving bijective-on-objects or fully-faithful morphisms, all of the properties enumerated in Definition~\ref{def:strictifiable} hold
...
\end{enumerate}
We will see that in each case the collection is strictifiable. Property \ref{item:boff} is proved as \ref{lem:(boff)_really_is}. Property \ref{item:ioofflff} is obvious for $\MMonCat$ and $\CCompCat$, and by definition (see Remark~\ref{rem:traced_2morphisms}) for $\TrCat$.  Property \ref{item:arrowMPO} is shown in Remark~\ref{rem:pres_joint_detect}. Property~\ref{item:left2adj} .. Property~\ref{item:pres_ff}.. Property~\ref{item:pres_MPO}.. Property~\ref{item:createSEs}..
