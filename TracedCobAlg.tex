...
A strong functor $F\colon \cat{C}\to \cat{C'}$ between object-free traced categories sends each object in $\cat{C}$ to the tensor product of finitely many objects in $\cat{C'}$; that is, it induces a function $\Ob F\colon\Ob\cat{C}\to\TM(\Ob \cat{C'})$, where $\TM\colon\Set\to\Set$ is the free monoid monad. Let $\Set_{\TM}$ denote the Kleisli category of this monad, so $\Ob F$ is identified with a morphism in $\Set_{\TM}$. By Lemma~\ref{lemma:monad_morphism}, there is a functor $(\LCob{\bullet})\colon\Set_{\TM}\to\CompCat$, \todo{Patrick: check this paragraph; is $(\LCob{\bullet})$ clear enough notation?}and we can compose it with $\Lax(-,\Set)$ to obtain a functor which we denote
...
the connection between traced categories, their string diagrams, and algebras on the operad
of cobordisms. In Section~\ref{sec:deducing} we prove~\ref{thm:TheoremB}
and~\ref{cor:CorollaryB}, which compare lax set-valued functors to strong bijective-on-objects
functors, respectively for compact and traced categories. Our main result,~\ref{thm:TheoremA},
...
\begin{warning}
For the rest of this artcile, whenever we discuss monoidal categories, we will mean symmetric monoidal categories. For example, see the definition of $\MMonCat$ above.
\end{warning}
...
There are free/forgetful adjunctions\todo{Do we need $\FT$ and $\UT$? Reduce clutter by removing it throughout this section.}
\begin{align*}
...
\begin{proposition}\label{prop:free_compact_is_Cob}
   The free compact closed category $\FC(\cat{O})$ on a set $\cat{O}$ is equivalent to $\LCob{\cat{O}}$
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:local_equivalence}
   An equipment functor $F\colon\dcat{X}\to\dcat{Y}$ is a \emph{local equivalence} if the square
   \begin{equation*}
      \begin{tikzcd}
...
\begin{proof}
   ...
   the cube is a pullback, hence there is a unique $\tilde{F}$ making the cube commute.

   If $F$ is a local equivalence, then the front face is a pullback, in addition to the left and
   right faces. It follows that the back face is a pullback.
\end{proof}
...
\begin{lemma}
   Let $T$ be a traced category, $C$ a compact category, and $F\colon T\to\UCT C$ a traced functor
   such that the extension $\tilde{F}\colon\Int(T)\twoheadrightarrow C$ is bijective on objects. If
   we form the factorization in $\MonCat$
   \begin{equation*}
      \begin{tikzcd}[column sep=large]
         \UTM T \ar[r,hook,"\UTM\eta_T"] \ar[d,two heads]
            & \UCM\Int(T) \ar[d,two heads,"\UCM\tilde{F}"] \\
         M \ar[r,hook] & \UCM C
      \end{tikzcd}
   \end{equation*}
   then:
         ...
         \begin{equation*}
            \begin{tikzcd}[column sep=large]
               T \ar[r,hook,"\eta_T"] \ar[d,two heads,"G"']
                  & \UCT\Int(T) \ar[d,two heads,"\UCT\tilde{F}"] \\
               T' \ar[r,hook,"H"'] & \UCT C
            \end{tikzcd}
         \end{equation*}
      \item there is an isomorphism $\alpha\colon C\iso\Int(T')$ such that $\alpha H=\eta_{T'}$.
   \end{compactitem}
\end{lemma}
\begin{proof}
   ...
   \begin{equation*}
      \begin{tikzcd}[column sep=large]
         T \ar[r,hook,"\eta_T"] \ar[d,two heads,"G"']
            & \UCT\Int(T) \ar[d,two heads,"\UCT\tilde{F}"']
               \ar[ddr,bend left=25,"\UCT\Int(G)"] &[between origins] \\
         T' \ar[r,hook,"H"] \ar[drr,hook,bend right=15,"\eta_{T'}"']
            & \UCT C \ar[dr,"\alpha" inner sep=1pt] & \\[-10]
         && \Int(T')
      \end{tikzcd}
   \end{equation*}
   By Lemma~\ref{lem:Int_preserves_bo}, we know that $\UCT\Int(G)$ is $\bo$, and therefore $\alpha$
   is also. But by the third part of Lemma~\ref{lemma:fully_faithful_and_trace}, $\alpha$ is $\ff$.
   Hence $\alpha$ is an isomorphism, completing the proof.
\end{proof}
...
\begin{proposition}\label{Prop:mon_prof_equivalence}
   The functors $F$ and $U$, from (\ref{eqn:functors 1C CC}), induce an equivalence of fibrations
   \begin{equation*}
...
\end{align*}
given by (\ref{dia:TMvsFrMonCat}), Proposition~\ref{prop:free_compact_is_Cob}, Definitions~\ref{def:copresheaves},~\ref{def:FrMonProf_FrCompProf},~and~\ref{def:FrMonCompProf}, and Lemma~\ref{lem:FrMonCompProf_Psh_Mon}.
\end{proof}
...
\begin{proof}
By Definition~\ref{***}, the left-hand side is equivalent to $\CPsh(\dTrProf)$. \todo{Finish proof.}
\end{proof}
