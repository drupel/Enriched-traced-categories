...
\end{lemma}
\begin{proof}
   By the universal property of $\Path{f}$ there is a unique morphism $s\colon a\to \Path{f}$ such that
   \begin{equation}
      \begin{tikzcd}[column sep=small]
...
\begin{itemize}
   ...
   \item There is a locally faithful functor $U\colon\ccat{D}\to\Cat$ which preserves fully-faithful
   morphisms, and which has a left 2-adjoint $F\colon\Cat\to\scat{D}$.
   \item $U$ creates mapping path objects and surjective equivalences.
\end{itemize}
