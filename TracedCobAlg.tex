...
We will see that $\CCompCatStrong$ has arrow objects, but they are not preserved by the 2-functor $\CCompCatStrong\to\MMonCatStrong$. For a compact category $\cat{C}$, the arrow category $\cat{C}^2$ has as objects the \emph{isomorphisms} $a\To{\iso} b$ in $\cat{C}$, and as morphisms the commuting squares. This is compact: the dual of $a\To{f} b$ is $a^\ast\To{(f^{-1})^\ast}b^\ast$. 
\todo{finish}
\end{example}
