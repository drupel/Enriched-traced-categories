...
labels $\cat{O}$.\todo{Change $\cat{O}$ to something else throughout? Perhaps $\mathsf{L}$ or $\mathfrak{L}$ for label?}
...
\todo[inline]{Combine this section with \ref{sec:traced_string_cob}?}
...
\begin{lemma}
   ...
   2-cell is a monoid homomorphism. In other words, $\MOb_0\colon\Mon(\dcat{D})\to\dcat{D}_0$ is a
   fibration, and there is a morphism of fibrations
   \begin{equation*}
      \begin{tikzcd}[column sep=tiny]
         \Mon(\dcat{D}) \ar[dr,two heads,"\MOb_0"'] \ar[rr]
...
\begin{definition}
   Let $\dcat{D}$ be an equipment. There is a fibration $\Pt(\dcat{D})\twoheadrightarrow\dcat{D}_0$,
   ...
   The fibration morphism $\Mon(\dcat{D})\to\dcat{D}_1$ factors through $\Pt(\dcat{D})$ in the
   evident way.
\end{definition}
...
\begin{equation}\label{eqn:primary_goal}
   \begin{tikzcd}
      \CPsh(\dCompProf) \equiv \Pt(\dCompProf) \equiv \Mon(\dCompProf).
   \end{tikzcd}
\end{equation}
over $\CompCat$.
...
\begin{proposition}\label{prop:unit_implies_monoid}
   The forgetful functor $\Mon(\dCompProf)\to\Pt(\dCompProf)$ is an equivalence of fibrations.
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{Prop:canonical unit}
   ...
   factors through $\Pt(\dCompProf)$, and hence also through $\Mon(\dCompProf)$.
\end{proposition}
\begin{proof}
   Given any $f\colon c\to d$ in $\cat{C}$, we must define an element $i(f)\in FM(c,d)=M(c^*\otimes
...
\begin{lemma}\label{Lem:comp prof bijection}
   Given any $N\in\Pt(\dCompProf)$, there is a canonical bijection $N(a,b)\iso N(I,a^*\otimes b)$ for
   any objects $a,b\in\cat{C}$.
\end{lemma}
\begin{proof}
...
%   functor $\FM\cat{O}\to\FM\cat{O}'$. Hence, $\dTrProf$ is usefully thought of as the double
