...
\subsection{The category of props}

One can think of props as symmetric monoidal categories that are free on a fixed \emph{label set} (also known as its set of colors). Functors between props need to descend to a function between label sets. Thus one can define the category $\Prop$ of props and prop functors as the pullback
$$
\begin{tikzcd}
\Prop\ar[r]\ar[d,"\Ob"']&\SymMonCat\ar[d,"\Ob"]\\
\Set\ar[r,"\Fun{List}"']&\Set
\end{tikzcd}
$$
where $\Fun{List}\colon\Set\to\Set$ is the free monoid monad.
