...
\begin{remark}
For an exact equipment $\dcat{D}$, we will indicate that a vertical morphism is in the left ($\bo$) class using a two-headed arrow $c\tto d$, and we will indicate the right ($\ff$) class using a hook arrow, $c\inj d$. Thus, the second part of Theorem~\ref{thm:orthogonal} says that for every solid-arrow diagram:
$$
\begin{tikzcd}
a\ar[d,two heads]\ar[r]&b\ar[d,hook]\\
c\ar[r]\ar[ur,dashed]&d
\end{tikzcd}
$$
there exists a dashed arrow as shown.

We sometimes also use the two-headed arrow to indicate fibrations of categories (e.g., as in Definition~\ref{def:local_equivalence}). These two uses of the symbol $\tto$ can be distinguished as follows. If the arrow occurs between objects in an equipment, then it indicates a $\bo$ map; if it occurs as a structural part of a given equipment\todo{Patrick: can this be better phrased?}, e.g., as in Definition~\ref{def:equipment}, then it indicates a fibration. 
