...
monoidal functors. Write $\TrCat_{\LabSet}$ for the subcategory of those traced categories $\cat{T}$ for which the monoid of objects is free on the set $\LabSet$, with identity-on-objects functors $\cat{T}\to\cat{T}'$ between them.
...
to the free compact category on $\LabSet$ \todo{This is saying $\LCob{\LabSet}$ is the free compact category on $\LabSet$, right?} (e.g.\ see \cite{KellyLaplaza,Abramsky2}).  We can compose
...
the objectwise-free traced categories.
...
\begin{remark}
Another way to address the condition of objectwise-freeness is to embrace it, and ask that functors between them be not only strict but also send generating objects to generating objects,  i.e.\ to consider PROPs rather than monoidal categories. In fact, there is an analogue of \ref{thm:TheoremA_statement} in which $\TrFrObCat$ is replaced by $\TrPROP$. We do not prove this result, but we will clarify it at an appropriate time; see Remark~\ref{rem:PROP}.
\end{remark}
...
objectwise-free. Let $\TrFrObCat$ and $\CompFrObCat$ (respectively) denote their underlying
1-categories.
...
objectwise-free traced (resp.\ compact) categories relate to general traced categories, we prove in
...
Section~\ref{sec:characterization_of_traced}) of the 2-category $\TTrCatStrong$ solely in terms of
...
\]
 Likewise, if $\cat{C}$ and $\cat{C}'$ are objectwise-free compact categories, then the object part $\Ob F$ of a strong functor $F\colon\cat{C}\to\cat{C}'$ can be identified with a morphism in the Kleisli category $\Set_{\TC}$.
...
freeness-on-objects needed for \ref{thm:TheoremA}. 

The appendix contains material that is not
...
Recall that a pseudo-pullback of a cospan $A_1\To{f_1}A\From{f_2}A_2$ is a diagram
\begin{equation}
...
\begin{lemma}
   ...
   $\dcat{D}_0$ have terminal objects, preserved by $F_0$. Then there is an induced
...
$M\colon c\tickar c$ to its underlying object $|M|\coloneqq c$. The following result can also be found in \cite{FioreGambinoKock}.\todo{P: Christina suggested we add this reference; thoughts?}
...
\begin{example}
   ...
   Let $\dSpan$ be the equipment defined in Example~\ref{ex:dspan}\todo{Span example was removed, this one should be also in light of Ex. 2.4.6, correct?.}. A monoid in $\dSpan$ consists of
...
Exact equipments arising in practice almost always have local coequalizers, and in this case it is possible to simplify the definition, as we show in Proposition~\ref{prop:exact_coeqs}. First, recall from Definition~\ref{def:equipment} the notion of companion $\comp{i}\colon a\tickar b$ and conjoint $\conj{i}\colon b\tickar a$ for any vertical map $i\colon a\to b$ in an equipment $\dcat{D}$; recall also the unit map $\unitOf{a}\colon a\tickar a$.


\begin{proposition}
    \label{prop:exact_coeqs}
  Let $\dcat{D}$ be an equipment with local coequalizers, and suppose $\dcat{D}$ satisfies Condition
  1 of Definition~\ref{def:exact_equipment}, giving $i_M\colon c\to\Col{M}$. Then $\dcat{D}$ satisfies Condition 2 if and only if
  ...
  \begin{equation}
    ...
    \end{tikzcd}
  \end{equation}
  or, equivalently, $\conj{i}_M\otimes_M\comp{i}_M\iso\unitOf{\Col{M}}$.
\end{proposition}
\begin{proof}
  ...
  action of $M$ on $\comp{i}_M$ and right action of $N$ on $\conj{i}_N$. Assuming
  $\conj{i}_M\otimes_M\comp{i}_M\iso\unitOf{\Col{M}}$, we can construct an inverse to this restriction
  ...
  \begin{align*}
    \conj{i}_M\otimes_M (\comp{i}_M\odot X\odot\conj{i}_N)\otimes_N\comp{i}_N
      ...
      &\iso \unitOf{\Col{M}}\odot X\odot \unitOf{\Col{N}} \\
      &\iso X \\
    \intertext{and}
  ...
  Conversely, assuming the functor \eqref{eqn:exact_Hor_Bimod} is an equivalence, then we can prove
  the isomorphism $\conj{i}_M\otimes_M\comp{i}_M\iso\unitOf{\Col{M}}$ by restricting both sides:
  \begin{align*}
    \comp{i}_M\odot(\conj{i}_M\otimes_M\comp{i}_M)\odot\conj{i}_M
      ...
      &\iso \comp{i}_M\odot\unitOf{\Col{M}}\odot\conj{i}_M. \qedhere
  \end{align*} 
\end{proof}
...
\eta_U$. More precisely, if $\sigma_{A,B}$ is the symmetry isomorphism, one defines $\Tr^U_{X,Y}[f]$ to be the composite
\begin{equation*}
...
\todo[inline]{Mention that we may be introducing these for the first time?}
...
This section aims to establish an equivalence between copresheaves on, and monoids in, compact (resp. traced) categories. See Definitions~\ref{def:copresheaves}~and~\ref{def:monoids}.
\begin{theorem}
...
\begin{proof}
  ...
  \begin{equation}
    \begin{tikzpicture}[string diagram]
      ...
      \draw[ar] (n1) to[in=0,looseness=2,"d"] (mid);
      \draw (mid.east) -- (mid.west);
      \draw[ar] (mid) to[out=180,in=180,looseness=2,"d"'] (n2);
...
\end{corollary}
We abuse notation slightly and denote each of the factorization systems above by $(\bo,\ff)$. Recall the adjunction \eqref{dia:traced_compact_adjunction} and write $\eta_{\cat{T}}\colon\cat{T}\to\UCT\Int(\cat{T})$ for the unit component on $\cat{T}\in\TTrCat$.
\begin{lemma}
      \label{lem:Tr_bo_Int}
...
\todo[inline]{We never said that $\LCob{\LabSet}$ is the free compact category on $\LabSet$!!!  Didn't we?  See comment in intro.}
...
are "objectwise-free''. To make this precise, we will further assume we have a 1-category
...
\bibitem{FioreGambinoKock}
 T.M.~Fiore; N.~Gambino; J.~Kock
Monads in double categories (English summary),  
J. Pure Appl. Algebra 215 (2011), no. 6, pp.~1174--1197. 
