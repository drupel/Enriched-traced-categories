...
\todo[inline]{Move \ref{def:kleisli_equipment}, \ref{def:DFbo}, and \ref{prop:objectfree_Mod_bo} to the beginnging of Section 3.5, "Monoids on free objects"}
Consider an equipment $\dcat{D}$ together with an adjunction to a category $\cat{S}$:
\begin{equation}
...
\todo[inline]{Fold the definition below into that above. Replace $\dcat{D}_{F}^{\bo}$ with $\dcat{D}_T^{\bo}$, replace $\psi_F$ with $\varphi_T^{\bo}$, and replace $(\psi_F)_0$ with $k_T^{\bo}$.}
...
\begin{definition}
      \label{def:DFbo}
   Let $\dcat{D}$ be an exact equipment with $k_T\colon\cat{S}_T\to\dcat{D}_0$ as above. Define the equipment $\dcat{D}_F^{\bo}$ as follows: the vertical category is defined by the strict pullback
   \[ \begin{tikzcd}
   ...
   \[ \begin{tikzcd}
      (\dcat{D}_F^{\bo})_1 \ar[r] \ar[dr,phantom,"\lrcorner" very near start] \ar[d, two heads]
