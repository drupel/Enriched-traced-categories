...
\end{equation}
Note that $\dcat{D}$ being exact does \emph{not} imply $\dcat{D}_T$ is exact.  

By Lemma~\ref{lemma:FFLE_Mod}, $\dMod(\varphi_T)\colon\dMod(\dcat{D}_T)\to\dMod(\dcat{D})$ is a fully faithful local equivalence.  If $\dcat{D}$ is exact, we can use the equivalence $\dMod(\dcat{D})\equiv\dcat{D}^{\bo}$ from Theorem~\ref{thm:Mod_vs_bo} to identify $\dMod(\dcat{D}_T)$ with the full subcategory of $\dcat{D}^{\bo}$ spanned by the objects in the image of $\dMod(\varphi_T)$, more precisely we have the following.
\begin{proposition}
...
\end{proposition}
\begin{proof}
   The first claim follows from Theorem~\ref{thm:Mod_vs_bo} and the definition of $\dcat{D}^{\bo}_F$.
...
\end{proposition}
\begin{proof}
   Suppose that $M\colon \cat{C}\tickar \cat{C}$ is a monoid in $\dMonProf$. As in the proof of
