...
on the set $\LabSet$, together with identity-on-objects functors. Let $\TTrFrObCat$ denote the full subcategory of $\TTrCat$ spanned by the traced categories that are object-free, and let $\TrFrObCat$ denote its underlying 1-category.
...
concatenation), together with identity-on-objects functors. Let $\CCompFrObCat$ denote the full subcategory of $\CCompCat$ spanned by the traced categories that are object-free, and let $\CompFrObCat$ denote its underlying 1-category. 
...
equivalent to that of object-free traced categories and strict functors (see Corollary~\ref{cor:TrCat_ObjectFree}). For
...
(respectively traced) categories which are ``object-free''.
Theorem~\ref{thm:object-free_strong_equivalence} provides the modification of
...
\begin{theorem}\label{thm:object-free_strong_equivalence}
...
The following corollary is a direct application of Theorem~\ref{thm:object-free_strong_equivalence}.

\begin{corollary}\label{cor:TrCat_ObjectFree}
   There are equipment functors (left) and underlying 2-functors (right)
   \begin{align*}
      \dMod(\dFrMonProf)  &\to \dMonProf  & \MMon(\dFrMonProf)  &\to \MMonCatStrong \\
      \dMod(\dFrTrProf)   &\to \dTrProf   & \MMon(\dFrTrProf)   &\to \TTrCatStrong  \\
      \dMod(\dFrCompProf) &\to \dCompProf & \MMon(\dFrCompProf) &\to \CCompCatStrong
   \end{align*}
...
\begin{proof}
   ...
   Theorem~\ref{thm:object-free_strong_equivalence}, taking $\ccat{C}$ to be $\CCat$ for the first
...
\begin{named}{Theorem A}\label{thm:TheoremA}
  ...
  \end{equation*}
  such that the induced 2-functor $\int^{\LabSet\in\Set_{\TT}}(\LCob{\LabSet})\alg\to\TTrCatStrong$ is
  2-essentially surjective.

  The above statements also hold when $\TT$ is replaced by $\TC$ and when $\TrCat$ and $\TTrCatStrong$ are replaced by $\CompCat$ and $\CCompCatStrong$.
\end{named}
