...
\end{tikzcd}
\]
satisfying some equations (see, e.g., \cite{Leinster,BorceuxV2}). If $\cat{C}$ and $\cat{D}$ are symmetric monoidal, then $F$ is
\emph{lax symmetric monoidal} if the $\mu$'s respect $\sigma$ in the appropriate way. If all $\mu$'s
are isomorphisms, then $F$ is \emph{strong}, while $F$ is \emph{strict} if the $\mu$'s are all
identities.
...
It is easy to check that a strong functor $F\colon\cat{C}\to\cat{M}$ preserves all duals that exist in $\cat{C}$, i.e., there is a natural isomorphism $F(c^*)\iso F(c)^*$. From this, it follows that if $F,G\colon\cat{C}\to\cat{C'}$ are functors between compact categories, then any natural transformation $\alpha\colon\cat{C}\to\cat{C'}$ is a natural isomorphism. Indeed, for any object $c\in\cat{C}$, the inverse of the $c$-component $\alpha_c\colon Fc\to Gc$ is given by $\alpha_{c^*}\colon Fc^*\to Gc^*$, or more precisely by 
$$
Gc\To{\eta_{Fc}} Gc\otimes Fc^*\otimes Fc\To{\alpha_{c^*}}Gc\otimes Gc^*\otimes Fc\To{\epsilon_{Gc}}Fc.
$$
...
\begin{example}
   ...
   If $(\cat{M},I,\otimes)$ is a monoidal category then $\cat{M}^2$ has a natural monoidal product
   $$
   	\cat{M}^2\times\cat{M}^2\iso(\cat{M}\times\cat{M})^2\To{\otimes^2}\cat{M}^2,
   $$
 and monoidal unit given by the identity map $\id_I$ on the unit of $\cat{M}$. The maps $\dom,\cod\colon\cat{M}^2\to\cat{M}$ are strict monoidal functors, and the transformation $\kappa\colon\dom\to\cod$ is monoidal as well. Because $\cat{M}^2$ is an arrow object in $\CCat$, it is easy to check, using the universal properties, that $\cat{M}^2$ is an arrow object in $\MMonCatStrong$.
...
\begin{example}
   ...
   $$
   \Ob P(F)\coloneqq\{(A,B,i)\|A\in\Ob\cat{A}, B\in\Ob\cat{B}, i\colon F(A)\To{\iso}B\text{ is an isomorphism}\}
   $$
 ...
 The mapping path object of a strong functor $F\colon\cat{M}\to\cat{M'}$between monoidal categories exists and is preserved by the forgetful functor $\MMonCatStrong\to\CCat$. Indeed, the mapping path object $P(F)$ of the functor between underlying categories has a canonical monoidal structure, and it...
...
$\iota$ as the inclusion of the strict morphisms into the strong morphisms.

For the remainder of this section we will make the following assumptions:
\begin{itemize}
   \item $\ccat{D}$ has a bijective-on-objects/fully-faithful factorization.
   \item $\ccat{D}$ has arrow objects.
   \item There is a functor $U\colon\ccat{D}\to\Cat$ which preserves fully-faithful morphisms, and
   which has a left 2-adjoint $F\colon\Cat\to\scat{D}$.
   \item $U$ creates mapping path objects and surjective equivalences, and reflects identity
   2-cells.
\end{itemize}
...
\begin{theorem}
   ...
   counit $q_A\colon QA\to A$ of this adjunction is given by factoring the counit of the $F\dashv U$
   adjunction:
   \begin{equation}
      \begin{tikzcd}
...
adjoint of the section $s_{Uf}\colon UA\to U(\Path{f})=\Path{UA}$ defined as in
Lemma~\ref{lem:mapping_path_equiv}. It follows by adjointness that the following diagram commutes:
\begin{equation*}
   \begin{tikzcd}
...
The lower triangle $\pi_A\circ\hat{f}''=q_A$ follows directly from \eqref{eq:f-hat_fact}.
