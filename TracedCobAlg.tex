...
\settypeblocksize{*}{37pc}{*} % {height}{width}{ratio}
...
\begin{abstract}
   ...
   \textbf{Keywords:} Traced categories, compact categories, lax functors, equipments, monoidal categories, operads, factorization systems.
\end{abstract}
...
The compact category $\Cob/\LabSet$ is varies functorially in $\LabSet\in\Set$, and we will see later that
...
fibers are equivalent, using \eqref{eq:single_fiber_tr} (respectively using \eqref{eq:single_fiber_comp})
to $\TrCat_{\LabSet}$ (respectively to $\CompCat_{\LabSet}$) for some fixed label set $\LabSet$. Our
...
\begin{remark}
   It would be too much to ask \eqref{eqn:tracedCobAlg} to be an equivalence of 1-categories, because $\TrCat$ is throws away, rather than quotients by, natural isomorphisms in $\TTrCat$. On the other hand, we cannot expect $\LCob{\LabSet}$ to say anything about 2-cells, so the statement of \ref{thm:TheoremA} is the best we can expect.
\end{remark}
...
Using these factorization systems, we can see that the domain functors
...
In Section~\ref{sec:traced categories} we review monoidal, traced, and compact categories, as well as the notion of orthogonal factorization systems. In Section~\ref{sec:background_equipments} we review equipments (also known as framed bicategories) from \cite{Shulman}, and exact equipments from \cite{Schultz2015}. In \ref{sec:internal_presheaves} we record the definition of copresheaves internal to an equipment, which we need to properly express our main theorems. We finally introduce the equipments of primary interest, $\dMonProf, \dTrProf$, and $\dCompProf$ in Section~\ref{chap:equipments_monoidal_profunctors}. We prove that they are exact in Section~\ref{sec:exactness_proofs} and, in Section~\ref{sec:special_CompProf}, prove the special properties about $\dCompProf$ which are at the core of our results. In Section~\ref{sec:monoids_on_free} we deal with object-freeness, which we need for \ref{thm:TheoremA}. We deduce our main results in Section~\ref{sec:deducing}.
...
\begin{equation}\label{eqn:primary_goal}
   ...
   \end{tikzcd}
\end{equation}
over $\CompCat$. See Propositions~\ref{prop:unit_implies_monoid} and \ref{Prop:mon_prof_equivalence}.
