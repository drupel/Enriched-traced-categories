...
For any monoidal category $\cat{M}$, we denote by $\cat{M}\alg=\ncat{Lax}(M,\Set)$ the category of lax
...
in~\cite{MacL--CTWM}. Let $\CCompCat$ (resp. $\CCompCatStrong$) denote the 2-category of compact categories and strict (resp. strong) functors, and let $\CompCat$
...
concatenation), together with identity-on-objects functors. Let $\CCompFrObCat$ denote the full subcategory of $\CCompCat$ spanned by the traced categories that are free-on-objects, and let $\CompFrObCat$ denote its underlying 1-category. 
...
equivalent to that of free-on-objects traced categories and strict functors. For
...
fixed label set $\LabSet$. 

To state the next theorem, we give names to the categories described above. Let 
$$\TTrFrObCat\ss\TTrCat \quad \text{and} \quad \CCompFrObCat\ss\CCompCat$$ 
...
The proof of this result can be found in Section~\ref{sec:deducing}. In order to see how the free-on-objects traced (resp. compact) categories relate to general traced categories, we prove the following result as Corollary \ref{cor:object_frees}, which is well-known but difficult to find in the literature:
...
\subsection{Generalization: Lax algebras on compact categories}\todo{Is it a generalization?}
...
Let $\CCat_\iso$ denote the 2-category (or if one prefers, (2,1)-category) of categories, functors, and natural isomorphisms. There are free/forgetful (strict) 2-adjunctions of 2-categories, as to the left:
\begin{align}
\nonumber
...
This section is mostly independent from the rest of the paper. It is really only used to prove Corollary~\ref{cor:object_frees}.
...
\begin{definition}\label{def:bijective-on-objects}
   ...
   If every morphism $f$ of $\ccat{C}$ can be factored into a
bijective-on-objects morphism followed by a fully-faithful morphism, then this forms an orthogonal
factorization system on $\ccat{C}$, and in this case we say that $\ccat{C}$ \emph{has a bijective-on-objects / fully faithful factorization system}.
\end{definition}
...
\begin{lemma}\label{lem:fully-faithful_surjective_equiv}
   Let $f\colon a\to b$ and $g\colon b\to a$ be morphisms in a 2-category such that $fg=1_b$. Then $f$
...
\subsection{Arrow objects and mapping path objects}
...
\begin{example}\label{ex:arrow_objects}
   The 2-categories $\CCat$, $\CCat_\iso$, $\MMonCatStrong$, $\TTrCatStrong$, and $\CCompCatStrong$ have arrow objects. Clearly for an object $\cat{A}\in\CCat$, the usual arrow category $\cat{A}^2$ of arrows and commutative squares, has the necessary universal property. Similarly, the arrow category of $\cat{A}$ in $\CCat_\iso$ is the category whose objects are isomorphisms in $\cat{A}$, and whose morphisms are commutative squares (in which the other morphisms need not be isos).
...
The 2-morphisms between traced categories are also defined to be isomorphisms (see Remark~\ref{rem:traced_2morphisms}). For a traced category $\cat{T}\in\TTrCatStrong$, the arrow object $\cat{T}^2$ has the isomorphisms in $\cat{T}$ as objects and commuting squares as morphisms; i.e., it too is preserved by the 2-functor $\TTrCatStrong\to\Cat_\iso$. To see the traced structure of $\cat{T}^2$, suppose given objects $a\colon A\To{\iso} A'$, $b\colon B\To{\iso} B'$, and $u\colon U\To{\iso} U'$, as well as a morphism $(f,g)\colon a\otimes u\to b\otimes u$ as in the diagram to the left
...
\begin{lemma}\label{lem:2-adjunction}
...
\begin{definition}\label{def:mapping_path_objects}
   ...
   $\pi_b(\beta,\gamma)=\gamma$.
      ...
      We say that $\ccat{C}$ \emph{has mapping path objects} if a mapping path object $c^2$ exists for each morphism $f\colon a\to b$ in $\cat{C}$.
\end{definition}

\begin{example}\label{ex:mapping_paths}
   The 2-categories $\CCat$, $\CCat_\iso$, $\MMonCatStrong$, $\TTrCatStrong$, and $\CCompCatStrong$ have mapping path objects. For a morphism $F\colon\cat{A}\to\cat{B}$ in $\CCat$, the mapping path category $P(F)$ is a cousin to the comma category $(F\downarrow\id_B)$: the objects are triples
   $$
   \Ob P(F)\coloneqq\{(A,B,i)\mid A\in\Ob \cat{A}, B\in\Ob \cat{B}, i\colon F(A)\To{\iso}B\text{ is an isomorphism}\}
   $$
 and a morphism $(A,B,i)\to (A',B',i')$ in $P(F)$ consists of a pair of morphisms $A\to A'$ in $\cat{A}$ and $B\to B'$ in $\cat{B}$ such that the evident diagram commutes. The 2-category $\CCat_\iso$  has exactly the same mapping path objects as $\CCat$.
 ...
 The arrow objects and mapping path objects for the 2-categories $\MMonCatStrong$, $\TTrCatStrong$, and $\CCompCatStrong$ were discussed in Examples~\ref{ex:arrow_objects}~and~\ref{ex:mapping_paths}. Each has a notion of cone, in fact a certain weighted limit cone in $\CCat$, though we will not discuss that notion here. We mentioned in passing that the structure morphisms for that cone were strict monoidal functors and that they ``preserve and jointly detect" strictness in the sense of Definition~\ref{def:preserve_jdetect_strictness} below. In particular, the 2-categories $\MMonCat$, $\TTrCat$, and $\CCompCat$ also have arrow objects and mapping path objects, and the inclusions of strict-into-strong (e.g., $\MMonCat\to\MMonCatStrong$) preserve them. Looking back at Examples~\ref{ex:arrow_objects}~and~\ref{ex:mapping_paths}, we see that the forgetful functors
 \begin{gather*}
 \UM\colon\MMonCat\to\CCat\qquad\UT\colon\TTrCat\to\CCat_\iso\qquad\UC\colon\CCompCat\to\Cat_\iso
 \end{gather*}
...
\begin{proof}
Let $R$ be either $\UM$, $\UT$, or $\UC$. Its underlying 1-functor has a left adjoint, constructed in \cite{Abramsky2}. We showed in Remark~\ref{rem:pres_joint_detect} that $\MMonCat$, $\TTrCat$, and $\CCompCat$ have arrow objects, which are preserved by $R$. The result follows by Lemma~\ref{lem:2-adjunction}.
...
\subsection{Strict vs. strong morphisms}\label{sec:strict_vs_strong}
...
\begin{definition}\label{def:preserve_jdetect_strictness}
 ...
 Let $\ccat{X}$ and $\ccat{Y}$ be 2-categories and let $i\colon\ccat{X}\to\ccat{Y}$ be a 2-functor that is identity-on-objects, faithful, and locally fully-faithful. We say that the triple $(\ccat{X},\ccat{Y},i)$ \emph{has mapping path objects} if $\ccat{Y}$ has mapping path objects as in Definition~\ref{def:mapping_path_objects} such that
 \begin{itemize}
 \item for any $f\colon a\to b$ in $\cat{Y}$, the structure morphisms $a\From{\pi_a}P(f)\To{\pi_b}b$ are in $\ccat{X}$, and
 \item the pair $(\pi_a,\pi_b)$ \emph{preserves and jointly detects morphisms in $\ccat{X}$} in the following sense: for any morphism $\ell\colon x\to P(f)$ in $\ccat{Y}$, we have that $\ell$ is in $\ccat{X}$ if and only if the compositions $\pi_a\circ\ell$ and $\pi_b\circ\ell$ are in $\ccat{X}$.
\end{itemize}
We say that the triple $(\ccat{C},\ccat{D},i)$ \emph{has arrow objects} if the analogous conditions hold.
\end{definition}
...
%    (equivalently,
%   $U\iota$ has a left 1-adjoint and preserves arrow objects; see Lemma~\ref{lem:2-adjunction}).
 \item\label{item:pres_ff}
   The functor $U\iota$ preserves fully-faithful morphisms (equivalently, $F$ preserves
   bijective-on-objects morphisms), and $U\iota$ also preserves bijective-on-objects morphisms.
...
\begin{remark}
   Other than those involving bijective-on-objects or fully-faithful morphisms, all of the properties enumerated in Definition~\ref{def:strictifiable} (namely, Properties~\ref{item:ioofflff}, \ref{item:arrowMPO}, \ref{item:left2adj}, \ref{item:pres_MPO}, \ref{item:reflids}, and \ref{item:createSEs}) hold
...
Suppose that the collection $(\scat{D},\pcat{D},\ccat{C},\iota,U)$ is defined as in one of the following cases:
\begin{itemize}
\item $\scat{D}=\MMonCat, \quad \pcat{D}=\MMonCatStrong, \quad \ccat{C}=\CCat$,
 where $\iota\colon\scat{D}\to\pcat{D}$ is the inclusion and $U\colon\pcat{D}\to\ccat{C}$ is the forgetful functor; 
\item $\scat{D}=\TTrCat, \quad \pcat{D}=\TTrCatStrong, \quad \ccat{C}=\CCat_\iso$, where $\iota\colon\scat{D}\to\pcat{D}$ is the inclusion and $U\colon\pcat{D}\to\ccat{C}$ is the forgetful functor (recall that $\CCat_\iso$ is the 2-category of categories, functors, and natural isomorphisms); or
\item $\scat{D}=\CCompCat, \quad \pcat{D}=\CCompCatStrong, \quad \ccat{C}=\CCat_\iso$, where $\iota\colon\scat{D}\to\pcat{D}$ is the inclusion and $U\colon\pcat{D}\to\ccat{C}$ is the forgetful functor.
\end{itemize}
We will see that in each case the collection admits strong morphism classifiers. 

Property \ref{item:boff} is proved as \ref{lem:(bo,ff)_really_is}. Property \ref{item:ioofflff} is obvious for $\MMonCat$ and $\CCompCat$, and by definition (see Remark~\ref{rem:traced_2morphisms}) for $\TrCat$.  Property \ref{item:arrowMPO} is shown in Remark~\ref{rem:pres_joint_detect}. Property~\ref{item:left2adj} is shown in Corollary~\ref{cor:2_adjunctions_MonTrComp}. Property~\ref{item:pres_ff} is shown in Proposition~\ref{prop:boff_well_named}. Property~\ref{item:pres_MPO} is shown in Remark~\ref{rem:pres_joint_detect}. Property~\ref{item:reflids} is obvious: if $\alpha\colon F\to G$ is a 2-cell in $\scat{D_s}$ whose underlying natural transformation (in $\CCat$) is the identity then it is the identity. It remains to prove Property~\ref{item:createSEs}; we first treat the case $\scat{D}=\MMonCat$.

Suppose that $F\colon A\to B$ is a strict monoidal functor and that there is a surjective equivalence $g\dashv U\iota(F)$ in $\CCat$. Let $f\colon a\to b$ denote $U\iota(F)$, so $g\colon b\to a$. By Definition~\ref{def:surjective_equivalence}, we have a 2-cell $\epsilon\colon gf\iso 1_a$ and $1_b=fg$, $\epsilon g=1_g$ and $f\epsilon=1_f$. A strong functor $G\colon B\to A$ with $UG=g$ of course acts the same as $g$ on objects and morphisms. Thus it suffices to give the coherence isomorphisms $\mu\colon I_A\To{\cong}G(I_B)$ and $\mu_{x,y}\colon G(x)\otimes G(y)\To{\cong} G(x\otimes y)$ for objects $x,y\in B$, and see that they satisfy the required equations. Define $\mu$ to be the composite $I_A\To{\epsilon^{-1}}gf(I_A)=g(I_B)$, and define $\mu_{x,y}$ to be the composite
$$
gx\otimes gy\To{\epsilon^{-1}}gf(gx\otimes gy)=g(fgx\otimes fgy)=g(x\otimes y).
$$
The requisite equations can be checked by direct computation, though they actually follow from a more general theory (doctrinal adjunctions); see \cite{Kelly}.

Property~\ref{item:createSEs} holds for the case $\scat{D}=\CCompCat$ because it is a full subcategory of $\MMonCat$. For the case $\scat{D}=\TTrCat$, suppose given a strict traced functor $F\colon A\to B$, and let $G\colon B\to A$ be the associated monoidal functor constructed above. To see that it is traced, note that $B\To{G}A\To{F}B$ is the identity, so $G$ is fully faithful, and the result follows from Lemma~\ref{lemma:fully_faithful_and_trace}.
...
\begin{theorem}\label{thm:strong_classifier}
   Suppose that $(\scat{D},\pcat{D},\ccat{C},U,\iota)$ admits strong morphism classifiers. Then the functor $\iota$ has a left 2-adjoint $Q\colon\pcat{D}\to\scat{D}$. The
...
$\iota$. It then follows from Lemma~\ref{lem:2-adjunction} that $Q$ extends to a 2-functor which is
left 2-adjoint to $\iota$, completing the proof of the theorem.
...
\begin{corollary}\label{cor:object_frees}
 $\CompFrObCat$ is equivalent to $\CCompCatStrong$, etc.\todo{fill in}
\end{corollary}
...
\begin{lemma}\label{lem:(bo,ff)_really_is}
   ...
   Definitions~\ref{def:fully-faithful}, \ref{def:bijective-on-objects}.
\end{lemma}
\begin{proof}
   It is easy to see that the condition in Definition~\ref{def:fully-faithful} of $f$ being
   ...
   by Definition~\ref{def:bijective-on-objects}.
\end{proof}
...
\begin{remark}
   ...
   These two uses of the symbol $\onto$ can be distinguished as follows. If the arrow occurs between
...
Theorem~\ref{thm:free-on-objects_strong_equivalence} provides the modification of
...
\begin{example}
   ...
   $\dMonProf^{\bo}_{\FM}$ as the equipment of free-on-objects monoidal categories. Indeed, this just
   says that a free-on-objects monoidal category can be identified with a set $\LabSet$, a monoidal
...
\begin{lemma}\label{lem:free-on-obs_inclusion}
   Let $\dcat{D}$ be an exact equipment and $\cat{S}$ a category, together with an adjunction
   $F\colon\cat{S}\leftrightarrows\dcat{D}_0:\!U$, and let $T=UF$ be the induced monad. Suppose
   moreover that $U(\bo)\subseteq\mathrm{iso}(\cat{S})$. Then the following composite is a fully
   faithful local equivalence:
   \begin{equation*}
      \begin{tikzcd}
...
\end{lemma}
\begin{proof}
   By Proposition~\ref{prop:objectfree_Mod_bo} and Theorem~\ref{thm:Mod_vs_bo}, it suffices to
...
2-category $\VVer(\dcat{D})$ is 2-equivalent to the sub-2-category spanned by the free-on-objects
objects.\todo{Fix!}

\begin{theorem}\label{thm:free-on-objects_strong_equivalence}
   Let $\dcat{D}$ be an exact equipment, $\pcat{D}$ and $\ccat{C}$ be 2-categories, and suppose we
   have a 2-functor $\iota\colon\VVer(\dcat{D})\to\pcat{D}$ and a 2-adjunction
   $F\colon\ccat{C}\leftrightarrows\VVer(\dcat{D}):\!U$, which together admit strong morphism classifiers (see Definition~\ref{def:strictifiable}).

   Suppose further that there is a 1-category $\cat{S}$ and 1-adjunction
   $\Disc\colon\cat{S}\leftrightarrows\ccat{C}_0:\!\Ob$ to the underlying 1-category $\ccat{C}_0$ of
   $\ccat{C}$, with $\Disc$ fully-faithful and such that a morphism $f$ in $\ccat{C}$ is $\bo$ if
   and only if $\Ob f$ is an isomorphism. Let $T=\Ob\circ U\circ F\circ\Disc$ be the induced monad
   on $\cat{S}$.

   Then the composition
   \begin{equation*}
      \begin{tikzcd}
         \MMon(\dcat{D}_T)\ar[r,"\MMon(\varphi_T)"]
            &[1.4em] \MMon(\dcat{D}) \ar[r,"\VVer(\Col{\textrm{--}})"]
            &[1.2em] \VVer(\dcat{D}) \ar[r,"\iota"]
            &[-0.5em] \pcat{D}
      \end{tikzcd}
   \end{equation*}
...
\end{theorem}
\begin{proof}
   Let $G$ be the composition $G\coloneqq\VVer(\Col{\textrm{--}})\circ\MMon(\phi_T)$. We need to
   show that $\iota\circ G$ is a local equivalence (induces equivalences of hom categories) and
   essentially surjective.

   By Lemma~\ref{lem:free-on-obs_inclusion}, $G$ is 2-fully-faithful (induces isomorphisms of hom
   categories). Hence, to show that $\iota\circ G$ is a local equivalence, it suffices to show that
   $\iota$ induces equivalences of categories $\VVer(\dcat{D})(A,B)\iso\pcat{D}(\iota A,\iota B)$
   for any $A$ and $B$ in the image of $G$. In fact, this will hold whenever $A$ is in the image of
   $G$. We say that $A$ is \emph{flexible} in this case: every strong morphism out of $A$ is
   isomorphic to a strict morphism.

   If $A\in\dcat{D}$ is in the image of $G$, then there exists an object $s\in\cat{S}$ and a $\bo$
   morphism $f\colon F(\Disc(s))\twoheadrightarrow A$ in $\VVer(\dcat{D})$. By the $F\dashv U$
   ...
   Theorem~\ref{thm:strong_classifier}, we obtain a unique lift $p'_A$ in the square
   \begin{equation*}
      ...
      \end{tikzcd}
   \end{equation*}
   By Lemma~\ref{lem:fully-faithful_surjective_equiv}, it follows that $q_A$ is an equivalence in
   $\VVer(\dcat{D})$. Hence composition with $q_A$ induces the left equivalence in
   \begin{equation*}
      \begin{tikzcd}
         \VVer(\dcat{D})(A,B) \ar[r,"\equiv"]
            & \VVer(\dcat{D})(Q\iota A,B) \ar[r,"\iso"]
            & \pcat{D}(\iota A,\iota B)
      \end{tikzcd}
   \end{equation*}
   and it is easy to check that the composition is precisely $\iota$ on hom categories.

   Finally, to see that $\iota\circ G$ is essentially surjective, consider an object $D\in\pcat{D}$.
   We know from Section~\ref{sec:strict_vs_strong} that in the factorization
   \begin{equation*}
      \begin{tikzcd}
      ...
      \end{tikzcd}
   \end{equation*}
   $\iota q_A$ is an equivalence in $\pcat{D}$. If we can show that $QA$ admits a $\bo$ morphism
   $F(\Disc(s))\twoheadrightarrow QA$ in $\VVer(\dcat{D})$ for some $s\in\cat{S}$, then $QA$ will
   be in the image of $G$, hence $\iota\circ G$ will be essentially surjective.
   ...
   \begin{equation*}
      ...
      \end{tikzcd}
   \end{equation*}
   showing that $QA$ is in the image of $G$.
\end{proof}


The following corollary is a direct application of Theorem~\ref{thm:free-on-objects_strong_equivalence}.

\begin{corollary}\label{cor:TrCat_ObjectFree}
   There are equipment functors (left) and underlying 2-functors (right)
   \begin{align*}
      \dMod(\dFrMonProf)  &\to \dMonProf  & \MMon(\dFrMonProf)  &\To{\simeq} \MMonCatStrong \\
      \dMod(\dFrTrProf)   &\to \dTrProf   & \MMon(\dFrTrProf)   &\To{\simeq} \TTrCatStrong  \\
      \dMod(\dFrCompProf) &\to \dCompProf & \MMon(\dFrCompProf) &\To{\simeq} \CCompCatStrong
   \end{align*}
   which are fully faithful local equivalences of equipments and 2-equivalences of 2-categories,
   respectively.
\end{corollary}
\begin{proof}
   This follows directly from Lemma~\ref{lem:free-on-obs_inclusion} and
   Theorem~\ref{thm:free-on-objects_strong_equivalence}, taking $\ccat{C}$ to be $\CCat$ for the first
   row and $\CCatIso$ for the second two, and taking $\cat{S}=\Set$ for all three. The conditions
   involving $\cat{S}$ are obvious, and the rest of the assumptions are checked in
...
\begin{proof}
   ...
   \begin{equation*}
	...
	\quad\text{and}\quad
      ...
      \end{tikzcd}
   \end{equation*}
   By Lemmas~\ref{lem:FrCompProf_Psh_Mon} and~\ref{lem:FrMonCompProf_Psh_Mon}, we have equivalences of 1-categories
   $$
   \CPsh(\dFrTrProf)\simeq\Mon(\dFrTrProf)\quad\text{and}\quad\CPsh(\dFrCompProf)\simeq\Mon(\dFrCompProf).
   $$ 
   By Corollary~\ref{cor:TrCat_ObjectFree} and Corollary~\ref{cor:object_frees}, we have equivalences of 2-categories
   $$
   \MMon(\dFrTrProf)\simeq\TTrFrObCat\quad\text{and}\quad\MMon(\dFrCompProf)\simeq\CCompFrObCat.
   $$
The result follows because $\TrFrObCat$ and $\CompFrObCat$ are the underlying 1-categories.
...
M.~Hasegawa, S-Y.~Katsumata, A note on the biadjunction between 2-categories of traced monoidal categories and tortile monoidal categories, Mathematical Proceedings of the Cambridge Philosophical Society 148 no. 1 (2010).
 ...
 \bibitem{Kelly}
 G.M.~Kelly, Doctrinal adjunction, Category Seminar (Proc. Sem., Sydney, 1972/1973), Lecture Notes in Math., Vol. 420, (1974) Springer, Berlin, pp. 257Ð280. 
