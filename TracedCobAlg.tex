...
\item Add appropriate 2-categorical language throughout.
\item Define the 2-category $\Cat{XFrObCat}$ as the 2-Grothendieck construction of $\op{\Set}\To{\Fun{XFr}}\op{\Cat{XCat}}\To{-/\XCat_{\bo}}\Cat{2-Cat}$
...
In our model for modular design, any way to chunk the small boxes inside the big one will give the same result. This is a reflection of the functoriality of $P\colon\Cat{Cob}\to\Set$, which says that commutative diagrams in $\Cat{Cob}$ are sent to commutative diagrams of sets. Nested structures, given by composition of such morphisms, may be useful for the kind of chunking that humans use to understand complex systems. \cite{Miller, George A. "The magical number seven, plus or minus two: some limits on our capacity for processing information." Psychological review 63.2 (1956): 81.}
...
\begin{proposition}\label{prop:coslice Xbo}
  There is a pseudo-functor $-/\XCat_{\bo}\colon\op{\XCat}\to\Cat{Cat}$ sending any $\cat{C}\in\XCat$ to the category $\cat{C}/\XCat_{\bo}$.
\end{proposition}
\begin{proof}
...
For a set $\cat{O}$, let $\Fun{FrMon}(\cat{O})$ denote the free symmetric monoid on $\cat{O}$. Let $\XCat$ continue to denote the category of strong $\Cat{X}$-functors between $\Cat{X}$-categories, where $\Cat{X}$ is monoidal, traced, or compact. Let $\Cat{XFrObCat}$ denote the category whose objects are $X$-categories $\cat{C}$ equipped with a set $\cat{O}$ and a bijection $\Ob(\cat{C})\iso\Fun{FrMon}(\cat{O})$. Morphisms in $\Cat{XFrObCat}$ are defined so that the functor $\Cat{XFrObCat}\to\Cat{XCat}$ is fully faithful. In \ref{**} we will prove that the functor $\Cat{XFrObCat}\to\Set$ given by $\cat{C}\mapsto\cat{O}$ is a fibration.\todo{Move this paragraph to a proposition using 2.5.2}
...
Let $\XCat$ and $\Cat{XFrObCat}$ be as above. There is a 2-equivalence:
$$
\begin{tikzcd}
\XCat\arrow[r, shift left=.5ex, "\Fun{XFrOb}"]&\Cat{XFrObCat}\arrow[l, shift left=.5ex,"U"]
\end{tikzcd}
$$
where $U$ is the forgetful functor. In particular, for every $\Cat{X}$-category $\cat{C}$ there is a strict $\Cat{X}$-category $\Fun{FrOb}(\cat{C})$, whose symmetric monoid of objects is free, and an equivalence in $\XCat$:
...
\end{tikzcd}
$$
For any $\Cat{X}$-category $\cat{C}$, we have a counit $\Fun{XFr}\Ob(\cat{C})\to\cat{C}$, which is surjective on objects. We define $\Fun{XFrOb}(\cat{C})$ to be the factorizing object for the factorization system from Lemma~\ref{lemma:factorization system}:
$$
\begin{tikzcd}[column sep=small]
...
\end{tikzcd}
$$
The right-hand functor is fully faithful and clearly surjective, so it is an equivalence of categories. This proves that $\Fun{XFrOb}\circ\fun{U}\iso\id$, and because $U$ is a monomorphism, we have $U\circ\Fun{XFrOb}\iso\id$ as well.

\todo{Finish proof (2-categorical).}
...
$$\Lax(\cat{C},\Set)\simeq\cat{C}/\CompCat_{\bo}$$
and the equivalence (\ref{dia:traced lax strong}) follows from Proposition~\ref{prop:slice boo int}. All these constructions are natural in $\cat{C}$. \todo{Pretty this up.}
...
For any symmetric monoid $X$, let $\TrCat_{\boo X}$ denote the full subcategory of $\TrCat_{\bo}$ spanned by those traced categories whose monoid of objects is isomorphic to $X$.

\begin{proposition}\label{prop:natural equivalence cob-O and traced-O}

For any set $\cat{O}$, let $\Fun{FrMon}(\cat{O})$ denote the free symmetric monoid $\cat{O}$. There is an equivalence of categories 
...
This is an application of \ref{cor:Corollary B} in the case that $\cat{D}\coloneqq\Fun{TrFrOb}(\cat{O})$ is the free traced category on the set $\cat{O}$. By Proposition~\ref{prop:free traced and compact}, the objects of $\cat{D}$ are symmetric lists, $\Ob\cat{D}\iso\Fun{FrMon}(\cat{O})$. With the addition of Proposition~\ref{prop:free compact is Cob}, we have that $\Cat{Cob}/\cat{O}\iso\Int(\cat{D})$ is the free compact category on $\cat{O}$. Since all of these results are natural in $\cat{O}$, so too is the desired equivalence:
$$(\Cat{Cob}/\cat{O})\alg=\Lax(\Cat{Cob},\Cat{Set})\simeq\cat{D}/\TrCat_{\bo}=\TrCat_{\boo\Fun{FrMon}(\cat{O})}$$\todo{rewrite proof using new language}
