...
\begin{example}\label{ex:monoid_in_Prof}
   ...
   $\bullet$ is unital with respect to $i$. In fact, it is easy to see that equations \eqref{eq:Prof_monoid_A} and \eqref{eq:Prof_monoid_B}
...
\begin{lemma}\label{lem:(bo,ff)_really_is}
   ...
   Definitions~\ref{def:fully-faithful}, \ref{def:bijective_on_objects}.\todo{uses appendix}
\end{lemma}
\begin{proof}
...
For intuition, see Definition~\ref{def:free_on_objects}. The following proposition follows from Theorem~\ref{thm:Mod_vs_bo} and Lemma~\ref{lemma:FFLE_Mod}:
...
\begin{proof}
   ...
   Lemma~\ref{lem:free-on-obs_inclusion}, letting $\cat{S}=\Set$. The isomorphisms follow directly from Proposition~\ref{prop:objectfree_Mod_bo} and Definition~\ref{def:free_on_objects}.
\end{proof}
...
\begin{proof}
Let $R$ be either $\UM$, $\UT$, or $\UC$. Its underlying 1-functor has a left adjoint, constructed in \cite{Abramsky2}. We showed in Remark~\ref{rem:pres_joint_detect} that $\MMonCat$, $\TTrCat$, and $\CCompCat$ have arrow objects, which are preserved by $R$. The result follows by Lemma~\ref{lem:2_adjunction}.
...
\begin{proof}
   ...
   Lemma~\ref{lem:fully_faithful_surjective_equiv} it suffices to construct a section of $Uq_A$. We can
   ...
   $\iota$. It then follows from Lemma~\ref{lem:2_adjunction} that $Q$ extends to a 2-functor which is
