...
\end{tikzcd}
\]
where $\SSymMonCat_1$ (resp. $\TrCat_1$) is the underlying 1-category of $\SSymMonCat$ (resp.\ of $\TrCat$), and where $\Fun{List}\colon\Set\to\Set$ is the free monoid monad. This definition of $\Prop$ agrees with that in~\cite{HackneyRobertson}.\todo{Check this.}
...
\todo[author=Patrick,inline]{Examples: $\MMon(\dSpan)\iso\dProf$, $\MMon(\dSpan(\Mon))\iso\dMProf$}
...
\end{tikzcd}
\]
\todo[author=Patrick,inline]{Using equipment language, we can restate this entire diagram to be variable in $\cat{C}$.}
