...
In $\dMonProf$, $\dTrProf$, and $\dCompProf$, a vertical map is $\ff$ (resp. $\bo$) if and only if it is fully faithful (resp. bijective-on-objects) in the usual sense. 
\end{proposition}

\begin{proof}\todo{P: Check proof.}
In all three cases, the fully faithful functors are right-orthogonal to the bijective-on-objects functors, so by Theorem~\ref{thm:orthogonal} it suffices to show that bijective-on-objects coincides with $\bo$. It is clear by the construction of collapses in $\dMonProf$ that a map $f\colon\cat{C}\to\cat{D}$ is $\bo$ if and only if it is bijective-on-objects. The result follows because the collapse of a monoid in $\dCompProf$ and $\dTrProf$ is defined as the collapse of the underlying monoid in $\dMonProf$.
\end{proof}
...
\todo[author=David,inline]{How's this?}
