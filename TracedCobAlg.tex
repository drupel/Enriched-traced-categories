...
\begin{definition}
      \label{def:ptd}
   Given an equipment $\dcat{D}$ there is a fibration $\Ptd(\dcat{D})\twoheadrightarrow\dcat{D}_0$ whose objects are \emph{pointed proarrows}, i.e.\ proarrows $M\colon c\tickar c$ in $\dcat{D}$ equipped with a unit $i_M\colon c\to M$ as in \eqref{eqn:unit_and_mult} (but not a multiplication), and whose morphisms are 2-cells which preserve the units. We refer to objects in $\Ptd(\dcat{D})$ as \emph{pointed endo-proarrows}.
\end{definition}
