...
Let $\Cat{XCat}$, denote the category of $\Cat{X}$-strong functors between $\Cat{X}$-categories,  where $\Cat{X}$ is monoidal, traced, or compact. Then $\Cat{XCat}$ admits an orthogonal factorization system $(\cat{L},\cat{R})$, where the morphisms in $\cat{L}$ are bijective on objects, and the morphisms in $\cat{R}$ are fully faithful.
