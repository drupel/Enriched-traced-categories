...
\end{tikzpicture}
\end{equation}
represent compositions, i.e., new morphisms are constructed from old by specifying which outputs will be fed back into which inputs. In fact, these generalize Penrose diagrams in $\Cat{Vect}$ and the word \emph{traced} originates in this vector space terminology.  
...
By \cite{Abramsky}, $\Cat{Cob}/\cat{O}$ is the free compact category on the set $\cat{O}$ of objects. The objects in $\Cat{Cob}/\cat{O}$ are \todo[color=yellow]{this is not the same is the free commutative group generated by $\cat{O}$, because $[x,-x]\neq 0$. --David} signed symmetric lists in $\cat{O}$, the set of which we denote by $\Ob(\Cat{Cob}/\cat{O})=\SSList(\cat{O})$. Theorem~\ref{thm:compact lax and strong} amounts to the following equivalence of categories, when $\cat{C}=\Cat{Cob}/\cat{O}$:
...
There are other ways to define $\cat{V}$-enriched traced and compact categories, e.g., using the theory of framed bicategories. The above definition agrees, \todo{True?}though we do not prove this result in the present \todo[color=yellow]{Let's explore whether it is worth including a proof.}paper.

\end{remark}

\section[Applications of the cobordism-algebra perspective]{Applications of the cobordism-algebra perspective in engineering design}\todo{Kill this section?}
...
In this section we prove that an orthogonal factorization system exists on the categories $\SymMonCat$ and $\TrCat$ of symmetric monoidal (resp. traced) categories and strong functors (resp. traced functors; see Definition~\ref{def:traced functor}). 
...
\end{tikzcd}
$$
such that the left-hand functor is bijective on objects and the right-hand functor is fully faithful. If $\Cat{X}$ is ``traced", then \cite[Proposition 2.4]{Joyal-Street-Vertity} implies that there is a unique traced structure on $(\cat{F},\otimes_{\cat{F}})$. The proof of orthogonality is straightforward.\todo{None of this has been rigorously checked.}
...
\begin{proof}
This is straightforward, but the calculation is \todo{should be something like this} shown below:
\begin{align*}
...
\begin{corollary}\todo{relabel this Corollary 1.1.2}
Suppose that $\cat{D}$ is a traced category and its compact closure is $\cat{C}=\Int\cat{D}$. Then there is an equivalence of categories
\begin{align}\label{dia:traced lax strong}
\Lax(\cat{C},\Cat{Set})\iso\cat{D}/\TrCat_{\boo}
\end{align}
...
\begin{lemma}\label{Lem:mon profs equal boo functors}
	Let $\cat{C}$ be a monoidal category. There is an equivalence of categories $\Mon(\MProf(\cat{C},\cat{C}))\iso (\cat{C}/\MonCat)_{b.o.o.}$ between the category of monoids in $\MProf(\cat{C},\cat{C})$ and the full subcategory of the coslice category $\cat{C}/\MonCat$ spanned by the bijective-on-objects functors.
\end{lemma}
...
Theorem~\ref{thm:compact lax and strong}, the equivalence
\[
  \Lax(\cat{C},\Cat{Set})\cong\cat{C}/\CompCat_{\boo},
\]
now follows as a simple corollary of proposition~\ref{Prop:mon prof equivalence} and lemma~\ref{Lem:mon profs equal boo functors}, together with the easy observation that if $\cat{C}$ is a compact category, then $\cat{C}/\MonCat_{\boo}\iso\cat{C}/\CompCat_{\boo}$ (if $F\colon\cat{C}\to\cat{X}$ is a monoidal functor and $a\in\cat{C}$ has a dual, then $Fa$ has a dual in $\cat{X}$).
