...
$$\Lax(\Cat{Cob}/\cat{O},\Cat{Set})\iso\Cat{CompCat}_{\Ob\cong\SSList(\cat{O})}.$$
...
\Lax(\cat{C},\Cat{Set})\iso\cat{D}/\Cat{TrCat}_{\Ob\iso}
\end{align}
This equivalence is natural in the traced category $\cat{D}$, using the factorization system on $\Cat{CompCat}$ from Lemma~\ref{lemma:factorization system}.
\end{corollary}
...
\section{The equivalence $\Cat{TrCat}\simeq(\Cat{Cob}/\bullet)\alg$}\label{sec:first equivalence}
...
For any set $\cat{O}$, let $\SList(\cat{O})$ denote the set of symmetric lists in $\cat{O}$. There is an equivalence of categories 
 $$(\Cat{Cob}/\cat{O})\alg\simeq\Cat{TrCat}_{\Ob\iso\SList(\cat{O})}$$
\end{theorem}
\begin{proof}
 Dylan....
\end{proof}
...
\begin{compactitem}
	...
	\item The Int construction $\Int\colon\Cat{TrCat}\to\Cat{CompCat}$ is left 2-adjoint to $U$. For any traced category $\cat{C}$, the unit $\cat{C}\to\Int(\cat{C})$ is fully faithful.
\end{compactitem}
...
**One direction is Int, the other is the factorization system on TrCat.
