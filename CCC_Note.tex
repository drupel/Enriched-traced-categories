...
$$\Lax(\Cat{Cob}/\cat{O},\Cat{Set})\iso\CompCat_{\Ob\cong\SSList(\cat{O})}.$$
...
\Lax(\cat{C},\Cat{Set})\iso\cat{D}/\TrCat_{\Ob\iso}
\end{align}
...
There are other ways to define $\cat{V}$-enriched traced and compact categories, e.g., using the theory of framed bicategories. The above definition agrees, \todo{True?}though we do not prove this result in the present paper.
...
\begin{align}\label{dia:compact to traced}
U\colon\CompCat\to\TrCat
\end{align}
...
For any set $\cat{O}$, let $\SList(\cat{O})$ denote the set of symmetric lists in $\cat{O}$. There is an equivalence of categories 
 $$(\Cat{Cob}/\cat{O})\alg\simeq\TrCat_{\Ob\iso\SList(\cat{O})}$$
\end{theorem}
\begin{proof}
 Dylan....
\end{proof}
...
\begin{compactitem}
	\item Let $\cat{D}$ be a traced category, and $F\colon\cat{C}\to\cat{D}$ a fully faithful symmetric monoidal functor. Then $\cat{C}$ has a unique trace for which $F$ is a traced functor.
	\item Any compact category has a canonical trace, defining a functor $U\colon\CompCat\to\TrCat$. See (\ref{dia:compact to traced}).
	\item The Int construction $\Int\colon\TrCat\to\CompCat$ is left 2-adjoint to $U$. For any traced category $\cat{C}$, the unit $\cat{C}\to\Int(\cat{C})$ is fully faithful.
\end{compactitem}

\begin{lemma}
	Let $\cat{D}$ be a compact category, and $F\colon\cat{C}\to\cat{D}$ a fully faithful strong functor. Then, using the unique trace on $\cat{C}$ making $F$ a traced functor, the functor $\Int(\cat{C})\to\cat{D}$ which is adjunct to $F$ is also fully faithful.
\end{lemma}
...
\begin{proposition}
	...
	\[
		(\cat{C}/\TrCat)_{\text{b.o.o.}} \simeq (\Int(\cat{C})/\CompCat)_{\text{b.o.o.}}
	\]
\end{proposition}

\begin{proof}

**To construct the functors: one direction is Int, the other is the factorization system on TrCat.
...
\begin{corollary}
	There is an equivalence of categories $\Lax(\cat{C},\Set)\simeq(\cat{C}/\CompCat)_{\text{b.o.o.}}$.
\end{corollary}
