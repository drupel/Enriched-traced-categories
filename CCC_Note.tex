...
Let $(\cat{M},\otimes,\tensor*[^{\cat{M}}]{\Tr}{})$ and $(\cat{N},\odot,\tensor*[^{\cat{N}}]{\Tr}{})$ be traced categories. A \emph{traced functor} is a strong monoidal functor $F\colon\cat{M}\to\cat{N}$, such that for any objects $X,Y,U\in\cat{M}$, the following diagram \todo[color=yellow]{left traces were defined above}commutes:
$$
\begin{tikzcd}
...
Every compact category $\cat{C}$ has a canonical trace structure, defined on a morphism $f\colon U\otimes X\to U\otimes Y$ to be \todo[color=yellow]{does this formula make sense as written? i.e. it doesn't seem to give a map $X\to Y$?}$\Tr^U_{X,Y}[f]:=\epsilon_U\circ f\circ \eta_U$. Thus we have a functor 
