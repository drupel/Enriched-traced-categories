...
$$
\begin{tikzpicture}[wiring diagram, bb min width=1.3cm] 
  \node[bb={2}{2}] (domain) {$f$};
...
\item For every $f\colon U\otimes X\to V\otimes Y$ and $g:V\to U$ we have
 ...
 \todo{Prettify this picture}
$$
\begin{tikzpicture}[wiring diagram, bb min width=1.3cm,baseline=(current bounding box.center)] 
  \node[bb={2}{2}, bb name =$f$] (X1) {};
  \node[bb={1}{1}, right=of X1, bb name = $g$] (X2) {};
...
\begin{tikzpicture}[wiring diagram, bb min width=1.3cm,baseline=(current bounding box.center)] 
  \node[bb={2}{2}, bb name = $f$] (X1) {};
  \node[bb={1}{1}, left=of X1, bb name = $g$] (X2) {};
...
\item For every $f\colon U\otimes X\to U\otimes Y$ and $g\colon W\to Z$ we have
 ...
 \todo{Draw a picture for this axiom.}
\end{itemize} 
...
$$
\begin{tikzcd}[column sep=small]
(\cat{C},\otimes_{\cat{C}})\arrow[dr, "bo"']\arrow[rr, "F"]&&(\cat{D},\otimes_{\cat{D}})\\
&(\cat{F},\otimes_{\cat{F}})\arrow[ur,"ff"']
\end{tikzcd}
$$
...
We now prove Theorem~\ref{thm:compact lax and strong} and Corollary~\ref{cor:traced lax and strong} as the following two corollaries.
 ...
 \begin{corollary}\label{cor:ccc's lax and strong}
 	There is an equivalence of categories $\Lax(\cat{C},\Set)\simeq(\cat{C}/\CompCat)_{bo}$.
 \end{corollary}
