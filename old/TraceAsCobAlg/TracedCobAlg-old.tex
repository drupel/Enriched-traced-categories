...
In fact, these generalize \todo{according to Wikipedia this is not accurate}Penrose diagrams in $\ncat{Vect}$~\cite{}, and the word \emph{traced} originates in this vector space terminology.
...
This is a morphism in the multicategory $\Cob$, underlying the symmetric monoidal category of oriented 1-cobordisms; see Section~\ref{sec:wds and cob}.
...
First, for each box $(\inp{X},\outp{X})$ that might appear in a string diagram, both $P$ and
$\cat{C}$ require a set, $P(\inp{X},\outp{X})$ and $\Hom_{\cat{C}}(\inp{X},\outp{X})$, respectively.
...
To give a sense of the equivalence in~\ref{thm:Theorem A} we will specify here how to construct a $(\Cob/\Ob\cat{C})$-algebra from a traced category $\cat{C}$.
...
\begin{named}{Theorem B}
   Let $\cat{C}$ be a compact category.  If $\Lax(\cat{C},\Set)$ denotes the category of lax
   monoidal set-valued functors out of $\cat{C}$, and $\cat{C}/\CompCat_{\bo}$ denotes the coslice
   category of compact categories and strong bijective-on-objects functors under $\cat{C}$, then
   there is an equivalence of categories
   \begin{equation}\label{dia:compact lax strong}
      \Lax(\cat{C},\Set)\simeq\cat{C}/\CompCat_{\bo}
   \end{equation}
   This is functorial in $\cat{C}$ in the sense of Theorem~\ref{PATRICK-todo}.
\end{named}
...
\begin{named}{Corollary B}
   For any traced category $\cat{D}$, there is an equivalence of categories
   \begin{equation}\label{dia:traced lax strong}
      \Lax(\Int(\cat{D}),\Set)\simeq\cat{D}/\TrCat_{\bo}
   \end{equation}
...
This is a reflection of the functoriality of $P\colon\Cob\to\Set$, which says that commutative diagrams in $\Cob$ are sent to commutative diagrams of sets.
Nested structures, given by composition of such morphisms, may be useful for the kind of chunking that humans use to understand complex systems~\cite{Miller}.

\subsection{Mathematical application: varying the operad}\label{sec:math application}

Formalizing modular design using operads, as in\todo{Are these in print yet?}\todo[author=David]{No, why do you ask?}~\cite{Spivak},~\cite{RupelSpivak}, and~\cite{VagnerSpivakLerman} was the original motivation for the present paper, as the drawings had strong similarities to those found in traced categories.
...
Let $\SSymMonCat$ denote the bicategory of symmetric monoidal categories and strong monoidal functors, and let $\Lax(\cat{C},\cat{D})$ denote the category of lax monoidal functors and monoidal transformations from $\cat{C}$ to $\cat{D}$.
Let $\CompCat$ denote the full subcategory of $\SSymMonCat$ spanned by the compact categories.
...
\end{tikzcd}
\]
where $\SSymMonCat_1$ (resp. $\TrCat_1$) is the underlying 1-category of $\SSymMonCat$ (resp.\ of $\TrCat$), and where $\List\colon\Set\to\Set$ is the free monoid monad. This definition of $\Prop$ agrees with that in~\cite{HackneyRobertson}.\todo{Check this.}
...
%
   ...
   %
   %\todo{Is any of this actually being used?}
   %
   %To continue we recall the constructions of various free monoidal structures on a category $\cat{C}$ following \cite{Abramsky2}.  These are defined inductively by successively including additional structure into the constructions.
   %
   ...
   %
   %By \cite{Abramsky2}, $\Cob/\cat{O}$ is the free compact category on the set $\cat{O}$ of objects. The objects in $\Cob/\cat{O}$ are signed symmetric lists in $\cat{O}$, the set of which we denote by $\Ob(\Cob/\cat{O})=\SSList(\cat{O})$. \ref{thm:Theorem B} amounts to the following equivalence of categories, when $\cat{C}=\Cob/\cat{O}$:\todo{This and the following remark probably don't go here.}
   %$$\Lax(\Cob/\cat{O},\Set)\iso\CompCat_{\boo\SSList(\cat{O})}.$$
   %
   %\begin{remark}
   %
   %Let $\cat{V}$ denote a symmetric monoidal category. To the best of our knowledge, neither the category of $\cat{V}$-enriched compact categories nor the category of $\cat{V}$-enriched traced categories has been defined in the literature. \ref{thm:Theorem B} and \ref{cor:Corollary B} could be used to motivate a definition of these categories by replacing $\Set$ with $\cat{V}$ in (\ref{dia:compact lax strong}) and (\ref{dia:traced lax strong}). That is, one could define a $\cat{V}$-enriched compact category to be a set $\cat{O}$ and a lax functor $\Cob/\cat{O}\to\cat{V}$.
   %
...
% Probably will cut this section
%-----
   % \section{A factorization system for strong functors}\label{sec:factorization system}

   % In this section we prove that an orthogonal factorization system exists on the categories $\SSymMonCat$, $\TrCat$, and $\CompCat$ of symmetric monoidal (resp.\ traced, compact) categories and strong functors (resp.\ traced functors; see~\cite{JoyalStreetVerity}).
   ...
   % \end{enumerate}
   % For example, there is an orthogonal factorization system on $\Cat$ in which the left class consists of functors that are bijective on objects and the right class consists of functors that are fully faithful.

   % \todo{Define $\Cat{X}$-categories and $\Cat{X}$-functors.}
   ...
   % \end{lemma}
   % \begin{proof}
   %  We first recall the factorization system $(\bo,\ff)$ on $\Cat$.
   ...
   %  Note that to establish the factorization system for $\XCat$, the only thing that needs to be checked is that the factorization in $\Cat$ lifts to $\XCat$.
   %  The isomorphism containment, closure under composition, and uniqueness of factorization all follow immediately from those same properties in $\Cat$.
   ...
   % \begin{tikzcd}
   %  \Set\arrow[r, shift left=.5ex, "\Fun{XFr}"]&\XCat\arrow[l, shift left=.5ex,"\Ob"]
   % \end{tikzcd}
   % \end{equation}
   ...
   %  Let $-/\XCat_{\bo}\colon\XCat\to\op{\Cat}$ be the functor from Proposition~\ref{prop:coslice Xbo}, and define $\Fun{XFrObCat}\colon\Cat{XKlSet}\to\op{\Cat}$ to be the composite
   %     $$\Cat{XKlSet}\To{i_{\Fun{X}}}\XCat\To{-/\XCat_{\bo}}\op{\Cat}.$$
   %  We define the fibration $\Cat{XFrObCat}\To{\pi_{\Fun{X}}}\Cat{XKlSet}$ to be the Grothendieck construction of $\Fun{XFrObCat}$.

   %  Sometimes we abuse notation and write $\Fun{XFrObCat}\colon\Set\to\op{\Cat}$ to denote what is actually the composite $\Fun{XFrObCat}\circ\eta_{\Fun{X}}$.
   % \end{definition}
   ...
   %        &\XCat\ar[u,bend right,"\id_{-}"']\ar[r,"-/\XCat_{\bo}"']&\op{\Cat}
   %  \end{tikzcd}
   %  $$
   ...
   %  $$
   %  \begin{tikzcd}
   %     \Set\arrow[r, shift left=.5ex, "i_{\Fun{X}}\circ\eta_{\Fun{X}}"]&\XCat\arrow[l, shift left=.5ex,"\Ob"]
   %  \end{tikzcd}
   %  $$
...
\todo[author=Patrick,inline]{Give examples: $\dProf$, $\dMProf$, $\dSpan$}
...
There is a double functor $\eta\colon\dcat{D}\to\dMod(\dcat{D})$ sending an object to the trivial monoid on it. There is also a (lax?) double functor $\mu\colon\dMod(\dMod(\dcat{D}))\to\dMod(\dcat{D})$ making $\dMod$ into a monad in the 2-category of framed bicategories.
...
If we want to modify the previous theorem to get an equivalence with $\dcat{D}$ rather than $\dcat{D}^{\bo}$, we need to restrict to objects of $\dcat{D}$ which are ``discrete''.
...
\todo[author=Patrick,inline]{This section and the next should be revisited after framed bicategory material}
...
%
   ...
   % \[
   %  \EndoProf\colon \op{\Cat} \to \MonCatLax.
   % \]
   ...
   %  There is an equivalence of categories $\Mon(\Prof(\cat{C},\cat{C}))\simeq \cat{C}/\Cat_{\bo}$ between the category of monoids in $\Prof(\cat{C},\cat{C})$ and the full subcategory of the coslice category $\cat{C}/\Cat$ spanned by the bijective-on-objects functors.
...
\todo[inline]{The following theorem, with naturality built in, is (\ref{eqn:primary goal}) + \ref{thm:Mod vs bo} on verts.}
...
\todo[inline]{See paragraph after \ref{thm:Verts}, and consult Patrick.}

\begin{proposition}\label{prop:slice boo int}
   Let $\cat{C}$ be a traced category.
   Then the Int construction provides an equivalence of categories
   \[
...
\begin{named}{Corollary B}\label{cor:Corollary B}
   ...
   \begin{equation*}
   \begin{tikzcd}
      \TrCat\ar[r,"\Int"]\ar[dr,"-/\TrCat_{\bo}"']
         &\CompCat\ar[d,"\alg"] \\
      &\Cat
   \end{tikzcd}
   \end{equation*}
...
\todo[inline]{All this will be replaced once 3.2 is finished}
...
\begin{proposition}\label{prop:natural equivalence cob-O and traced-O}
   Let $\cat{O}$ be a set.
   There is an equivalence of categories
      $$(\Cob/\cat{O})\alg\simeq\TrFrObCat(\cat{O})$$
...
\begin{proof}
   ...
   $$
   \begin{tikzcd}
      &\Set\ar[rr,"\CompFr\iso\Cob/-"]\ar[dl,"\eta"']\ar[dr,"\TrFr"]&&\CompCat\ar[dr,"\alg"] \\
      \ncat{TrKlSet}\ar[rr,"i"']&&\TrCat\ar[ur,"\Int"]\ar[rr,"-/\TrCat_{\bo}"']
         &&\Cat
   \end{tikzcd}
   $$
   ...
   For a set $\cat{O}$, the path across the top to $\Cat$ is $(\Cob/\cat{O})\alg$, which equals $\TrFrObCat(\cat{O})$, the path along the left and bottom.
\end{proof}
...
%\appendix
   ...
   %
   %\section{The easy direction $(\Cob/\bullet)\alg\to\TrCat$}
   %
   %\section{The functor $\TrCat\to(\Cob/\bullet)\alg$ on objects}
   %
   %
   ...
   % $$(\Cob/\cat{O})\alg\simeq\TrCat_{\boo\SList(\cat{O})}$$
   %\end{theorem}
   %\begin{proof}
   % Let $\cat{C}$ be the free traced category generated by $\cat{O}$.  We will define a $\Cob/\cat{O}$-algebra $\cat{P}=R(\cat{C})\colon\Cob/\cat{O}\to\Set$ as follows. For an object $X\in\Ob(\Cob/\cat{O})$, set
   ...
   %
   %
   %\section{The functor $\TrCat\to(\Cob/\bullet)\alg$ on morphisms}
%
