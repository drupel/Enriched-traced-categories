...
And surjections
\begin{align*}
thing_3\to\bigsqcup_{j\ge1}\feeddd{(\psi\circ_{2j}\phi)}{X}{X}
&\hsp\tn{and}\hsp
thing_4\to\bigsqcup_{j\ge1}\feeddd{(\psi\circ_{2j}\phi)}{X}{X}\\
...
thing_9\to\bigsqcup_{j\ge1}\feedcc{(\psi\circ_{2j}\phi)}{Z}{Z}
&\hsp\tn{and}\hsp
thing_{10}\to\bigsqcup_{j\ge1}\feedcc{(\psi\circ_{2j}\phi)}{Z}{Z}
\end{align*}
...
%The category $\mcB$, together with the map $\mcB\to\Cat$ given by $n\mapsto\ul{n}$, is a club. Any symmetric monoidal category $\mcM$ is an algebra on that club. That is, we have a category $\mcB\ltimes\mcM$ whose objects are pairs $(n,X)$, where $n\in\mcB$ and $X\taking\ul{n}\to\mcM$ is $n$ objects in $\mcM$. Its morphisms are pairs $(\sigma,f)$,  as in the natural transformation diagram:
%$$\xymatrix@R=12pt@C=40pt{
%\ul{m}\ar[rd]^X\ar[dd]_\sigma\\
%\ar@{}[r]|(.4){f\Down}&\mcM\\
%\ul{n}\ar[ru]_Y
%}
%$$
%Note that $f$ interacts with the loops in $\sigma$ by giving endomorphisms in $\mcM$.
%
%So far, this does not use the fact that $\mcM$ is symmetric monoidal, but for $\mcM$ to be a $\mcB$-algebra, we need a map $\mcB\ltimes\mcM\to\mcM$, which satisfies the algebra laws, and this encodes the SMC structure on $\mcM$.
%
%\begin{proposition}
%
%Let $\mcM$ be a symmetric monoidal category. Then $\mcB\ltimes\mcM$ is a traced symmetric monoidal category. If furthermore $\mcM$ is traced, then the functor $\mcB\ltimes\mcM\to\mcM$ preserves the trace in the sense that we have an isomorphism, natural in 
%
%
%\end{proposition}
